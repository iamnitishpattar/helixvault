...
export default function BioCompute() {
  ...
  const handleSearch = async (val, searchMode = mode) => {
    ...
    try {
      ...
      setResultsHistory(prev => [res.data.data, ...prev]);
    } catch (err) {
  ...
  return (
    <div className="container" style={{ padding: '3rem 1.5rem', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
        ...
        <h1 style={{ fontSize: '2.8rem', fontWeight: '800', marginBottom: '1rem', background: 'linear-gradient(135deg, #ffffff 0%, #e0e0e0 60%, #c8c8c8 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
      ...
      {resultsHistory.length > 0 && (
        <div style={{ animation: 'fadeIn 0.4s ease', display: 'flex', flexDirection: 'column', gap: '3rem' }}>
          {resultsHistory.map((results, historyIdx) => (
            <div key={historyIdx}>
              {/* Summary Bar */}
