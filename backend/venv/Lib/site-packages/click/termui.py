...
def prompt(
    ...
    while True:
        ...
        try:
            ...
            message = _mask_hidden_input(
                e.message, value) if hide_input else e.message
...
def confirm(
    ...
    while True:
        try:
            value = _readline_prompt(
                visible_prompt_func, prompt, err).lower().strip()
        except (KeyboardInterrupt, EOFError):
            raise Abort() from None
        if value in ("y", "yes"):
            rv = True
    ...
    * ``reset`` (reset the color code only)
    ...
    if fg:
        try:
            ...
            raise TypeError(
                _("Unknown color {colour!r}").format(colour=fg)) from None
    ...
    if bg:
        try:
            ...
            raise TypeError(
                _("Unknown color {colour!r}").format(colour=bg)) from None
