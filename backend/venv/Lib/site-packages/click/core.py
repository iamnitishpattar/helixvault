...
class Command:
    ...
    def get_params(self, ctx: Context) -> list[Parameter]:
        ...
        if __debug__:
            ...
            duplicate_opts = (
                opt for opt, count in opts_counter.items() if count > 1)
    ...
    def _main_shell_completion(
        ...
        rv = shell_complete(self, ctx_args, prog_name,
                            complete_var, instruction)
        sys.exit(rv)
...
class Parameter(ABC):
    ...
    def handle_parse_result(
        ...
        existing_default_explicit = ctx._param_default_explicit.get(
            self.name, False)
...
class Option(Parameter):
    ...
    def _parse_decls(
        ...
        for decl in decls:
            if decl.isidentifier():
                if name is not None:
                    raise TypeError(
                        _("Name '{name}' defined twice").format(name=name))
                name = decl
            else:
    ...
    def get_help_record(self, ctx: Context) -> tuple[str, str] | None:
        ...
        if "default" in extra:
            extra_items.append(_("default: {default}").format(
