...
class File:
    ...
    def _get_disk_file(self) -> BufferedRandom:
        ...
        if file_dir is not None and keep_filename:
            ...
            suffix = self._ext.decode(
                sys.getfilesystemencoding()) if keep_extensions else None
            ...
            try:
                tmp_file = cast(BufferedRandom, tempfile.NamedTemporaryFile(
                    suffix=suffix, delete=delete_tmp, dir=dir))
            except OSError:
                ...
                if ch == AMPERSAND:
                    if found_sep:
                        ...
                        if strict_parsing:
                            ...
                            self.logger.debug(
                                "Skipping duplicate ampersand at %d", i)
                    else:
        ...
        def advance_header_size(amount: int = 1) -> None:
            ...
            if current_header_size > self.max_header_size:
                raise MultipartParseError(
                    "Maximum header size exceeded", offset=i)
        ...
        while i < length:
            ...
            if state == MultipartState.START:
                ...
                if field.translate(None, TOKEN_CHARS):
                    ...
                    msg = "Found invalid character %r in header at %d" % (
                        bad, bad_i)
                    self.logger.warning(msg)
                ...
                if index == 0:
                    ...
                    if i0 >= 0:
                        ...
                        k = data.rfind(boundary[:1], max(
                            i, length - boundary_length + 1), length)
        ...
        if content_type == "application/octet-stream":
            ...
            parser = OctetStreamParser(
                callbacks={"on_start": on_start,
                           "on_data": on_data, "on_end": _on_end},
            ...
            def on_headers_finished() -> None:
                ...
                content_type = content_type_b.decode(
                    "latin-1") if content_type_b is not None else None
                if file_name is None:
                    f_multi = Field(field_name, content_type=content_type)
                else:
                    f_multi = File(file_name, field_name,
                                   config=self.config, content_type=content_type)
                    is_file = True
                ...
                transfer_encoding = headers.get(
                    b"content-transfer-encoding", b"7bit").lower()
...
def create_form_parser(
    ...
    form_parser = FormParser(content_type, on_field,
                             on_file, boundary=boundary, config=config)
...
def parse_form(
    ...
    if chunk_size < 1:
        raise ValueError(
            f"chunk_size must be a positive number, not {chunk_size!r}")
