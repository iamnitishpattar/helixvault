...
def find_prime_polys(generator=2, c_exp=8, fast_primes=False, single=False):
    ...
    if fast_primes:
        ...
        prim_candidates = [x for x in prim_candidates if x >
                           field_charac]  # filter out too small primes
    else:
    ...
    # you can use the following to print the hexadecimal representation of each prime polynomial: print [hex(i) for i in correct_primes]
...
def rs_correct_errata(msg_in, synd, err_pos, fcr=0, generator=2):
    ...
    err_eval = rs_find_error_evaluator(
        synd[::-1], err_loc, len(err_loc)-1)[::-1]
...
def rs_find_errata_locator(e_pos, generator=2):
    ...
    for i in e_pos:
        e_loc = gf_poly_mul(e_loc, gf_poly_add(
            _bytearray([1]), [gf_pow(generator, i), 0]))
    return e_loc
...
def rs_find_error_evaluator(synd, err_loc, nsym):
    ...
    _, remainder = gf_poly_div(gf_poly_mul(
        synd, err_loc), ([1] + [0]*(nsym+1)))
...
class RSCodec(object):
    ...
    def __init__(self, nsym=10, nsize=255, fcr=0, prim=0x11d, generator=2, c_exp=8, single_gen=True):
        ...
        # Memorize variables
        # number of ecc symbols (ie, the repairing rate will be r=(nsym/2)/nsize, so for example if you have nsym=5 and nsize=10, you have a rate r=0.25, so you can correct up to 0.25% errors (or exactly 2 symbols out of 10), and 0.5% erasures (5 symbols out of 10).
        self.nsym = nsym
        # maximum length of one chunk (ie, message + ecc symbols after encoding, for the message alone it's nsize-nsym)
        self.nsize = nsize
        ...
        # exponent of the field's characteristic. This both defines the maximum value per symbol and the maximum length of one chunk. By default it's GF(2^8), do not change if you're not sure what it means.
        self.c_exp = c_exp
        ...
        if single_gen:
            self.gen = {}
            self.gen[nsym] = rs_generator_poly(
                nsym, fcr=fcr, generator=generator)
        else:
            self.gen = rs_generator_poly_all(
                nsize, fcr=fcr, generator=generator)
    ...
    def encode(self, data, nsym=None):
        ...
        for chunk in self.chunk(data, self.nsize - self.nsym):
            enc.extend(rs_encode_msg(chunk, self.nsym, fcr=self.fcr,
                       generator=self.generator, gen=self.gen[nsym]))
        return enc
    ...
    def decode(self, data, nsym=None, erase_pos=None, only_erasures=False):
        ...
        for chunk in self.chunk(data, self.nsize):
            ...
            if erase_pos:
                ...
                erase_pos = [
                    x - self.nsize for x in erase_pos if x >= self.nsize]
            # Decode/repair this chunk!
            rmes, recc, errata_pos = rs_correct_msg(
                chunk, nsym, fcr=self.fcr, generator=self.generator, erase_pos=e_pos, only_erasures=only_erasures)
            dec.extend(rmes)
    ...
    def check(self, data, nsym=None):
        ...
        check = []
        for chunk in self.chunk(data, self.nsize):
            check.append(rs_check(chunk, nsym, fcr=self.fcr,
    ...
    def maxerrata(self, errors=None, erasures=None, verbose=False):
        ...
        if erasures is not None and erasures >= 0:
            ...
            if verbose:
                print('This codec can correct up to %i errors and %i erasures simultaneously' % (
                    maxerrors, erasures))
        ...
        if verbose:
            print('This codec can correct up to %i errors and %i erasures independently' % (
