...
class CLexer:
    ...
    def _handle_ppline(self) -> None:
        ...
        def success(pp_line: Optional[str], pp_filename: Optional[str]) -> None:
            if pp_line is None:
                self._error("line number missing in #line",
                            self._pos + line_len)
            else:
    ...
    def _handle_pppragma(self) -> List[_Token]:
        ...
        while pos < n and text[pos] != "\n":
            pos += 1
        if pos > start:
            toks.append(self._make_token(
                "PPPRAGMASTR", text[start:pos], start))
...
_bad_string_literal = '"' + _string_char + \
    "*" + _bad_escape + _string_char + '*"'
...
_regex_rules: List[_RegexRule] = [
    ...
    _RegexRule("U8STRING_LITERAL", _u8string_literal,
               _RegexAction.TOKEN, None),
    _RegexRule("U16STRING_LITERAL", _u16string_literal,
               _RegexAction.TOKEN, None),
    _RegexRule("U32STRING_LITERAL", _u32string_literal,
               _RegexAction.TOKEN, None),
    _RegexRule("STRING_LITERAL", _string_literal, _RegexAction.TOKEN, None),
    _RegexRule("HEX_FLOAT_CONST", _hex_floating_constant,
               _RegexAction.TOKEN, None),
    ...
    _RegexRule("INT_CONST_CHAR", _multicharacter_constant,
               _RegexAction.TOKEN, None),
