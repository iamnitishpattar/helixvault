...
class CParser:
    ...
    def _parse_any_declarator(
        ...
        if name_type is None or (
            ...
            if not allow_abstract:
                tok = self._peek()
                coord = self._tok_coord(
                    tok) if tok is not None else self.clex.filename
            ...
            if tok.type in _TYPE_SPEC_SIMPLE:
                ...
                spec = self._add_declaration_specifier(
                    ...
                    c_ast.IdentifierType(
                        [tok.value], coord=self._tok_coord(tok)),
                    "type",
                    append=True,
                )
                saw_type = True
    ...
    def _parse_struct_or_union_specifier(self) -> c_ast.Node:
        ...
        if self._peek_type() in {"ID", "TYPEID"}:
            ...
            if self._peek_type() == "LBRACE":
                ...
                self._expect("RBRACE")
                return klass(
                    name=name_tok.value, decls=decls, coord=self._tok_coord(
                        name_tok)
                )
        ...
        self._parse_error("Invalid struct/union declaration",
                          self._tok_coord(tok))
    ...
    def _parse_struct_declaration(self) -> Optional[List[c_ast.Node]]:
        ...
        if len(spec["type"]) == 1:
            ...
            return self._build_declarations(
                spec=spec, decls=[
                    dict(decl=decl_type, init=None, bitsize=None)]
            )
    ...
    def _parse_enumerator(self) -> c_ast.Node:
        ...
        if self._accept("EQUALS"):
            ...
            value = None
        enum = c_ast.Enumerator(name_tok.value, value,
    ...
    def _parse_decl_suffixes(self, decl: c_ast.Node) -> c_ast.Node:
        ...
        while True:
            if self._peek_type() == "LBRACKET":
                decl = self._type_modify_decl(
                    decl, self._parse_array_decl(decl))
                continue
    ...
    def _parse_direct_abstract_declarator(self) -> c_ast.Node:
        ...
        if lparen_tok:
            ...
            self._parse_error("Invalid abstract declarator",
                              self.clex.filename)
    ...
    def _parse_labeled_statement(self) -> c_ast.Node:
        ...
        match tok_type:
            ...
            case _:
                self._parse_error("Invalid labeled statement",
                                  self.clex.filename)
    ...
    def _parse_selection_statement(self) -> c_ast.Node:
        ...
        match tok.type:
            ...
            case _:
                self._parse_error(
                    "Invalid selection statement", self._tok_coord(tok))
    ...
    def _parse_iteration_statement(self) -> c_ast.Node:
        ...
        match tok.type:
            ...
            case _:
                self._parse_error(
                    "Invalid iteration statement", self._tok_coord(tok))
    ...
    def _parse_jump_statement(self) -> c_ast.Node:
        ...
        match tok.type:
            ...
            case _:
                self._parse_error("Invalid jump statement",
                                  self._tok_coord(tok))
    ...
    def _parse_constant(self) -> c_ast.Node:
        ...
        if tok.type in _INT_CONST:
            ...
            if u_count > 1:
                raise ValueError(
                    "Constant cannot have more than one u/U suffix.")
            if l_count > 2:
                raise ValueError(
                    "Constant cannot have more than two l/L suffix.")
...
_STORAGE_CLASS = {"AUTO", "REGISTER", "STATIC",
                  "EXTERN", "TYPEDEF", "_THREAD_LOCAL"}
