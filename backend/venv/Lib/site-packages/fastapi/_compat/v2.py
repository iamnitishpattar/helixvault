...
def get_definitions(
    *,
    ...
    validation_fields = [
        field for field in fields if field.mode == "validation"]
    ...
    for item_def in cast(dict[str, dict[str, Any]], definitions).values():
        if "description" in item_def:
            item_description = cast(
                str, item_def["description"]).split("\f")[0]
...
def serialize_sequence_value(*, field: ModelField, value: Any) -> Sequence[Any]:
    origin_type = get_origin(
        field.field_info.annotation) or field.field_info.annotation
    ...
    assert issubclass(origin_type, shared.sequence_types)
    # type: ignore[no-any-return,index]  # ty: ignore[invalid-return-type]
...
def create_body_model(
    *, fields: Sequence[ModelField], model_name: str
    ...
    field_params = {f.name: (f.field_info.annotation, f.field_info)
                    for f in fields}
    # type: ignore[call-overload]  # ty: ignore[no-matching-overload]
