...
WSProtocolType = Literal["auto", "none",
                         "websockets", "websockets-sansio", "wsproto"]
...
class Config:
    def __init__(
        ...
        log_config: dict[str,
                         Any] | str | os.PathLike[str] | RawConfigParser | IO[Any] | None = LOGGING_CONFIG,
        ...
        ssl_context_factory: Callable[[
            Config, Callable[[], ssl.SSLContext]], ssl.SSLContext] | None = None,
    ...
    def load(self) -> None:
        ...
        encoded_headers = [(key.lower().encode("latin1"), value.encode(
            "latin1")) for key, value in self.headers]
        ...
        if isinstance(self.ws, str):
            ws_protocol_class = import_from_string(
                WS_PROTOCOLS.get(self.ws, self.ws))
    ...
    def get_loop_factory(self) -> Callable[[], asyncio.AbstractEventLoop] | None:
        if self.loop in LOOP_FACTORIES:
            ...
            try:
                ...
                logger.error(
                    "Error loading custom loop setup function. %s" % exc)
                sys.exit(1)
    ...
    def bind_socket(self) -> socket.socket:
        ...
        if self.uds is not None:  # pragma: py-win32
            ...
            color_message = "Uvicorn running on " + \
                click.style(addr_format, bold=True) + " (Press CTRL+C to quit)"
