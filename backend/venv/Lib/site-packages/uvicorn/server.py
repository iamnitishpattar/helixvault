...
class Server:
    ...
    async def startup(self, sockets: list[socket.socket] | None = None) -> None:
        ...
        if sockets is not None:  # pragma: full coverage
            ...
            if os.path.exists(config.uds):
                uds_perms = os.stat(
                    config.uds).st_mode  # pragma: full coverage
    ...
    async def on_tick(self, counter: int) -> bool:
        ...
        if max_requests is not None and self.server_state.total_requests >= max_requests:
            logger.info(
                "Maximum request limit of %d exceeded. Terminating process.", max_requests)
            return True
    ...
    def capture_signals(self) -> Generator[None, None, None]:
        ...
        original_handlers = {sig: signal.signal(
            sig, self.handle_exit) for sig in HANDLED_SIGNALS}
        try:
            yield
        finally:
