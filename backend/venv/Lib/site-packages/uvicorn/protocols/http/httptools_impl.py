...
STATUS_LINE = {status_code: _get_status_line(
    status_code) for status_code in range(100, 600)}
...
class HttpToolsProtocol(asyncio.Protocol):
    def __init__(
        ...
        self.pipeline: deque[tuple[RequestResponseCycle,
                                   ASGI3Application]] = deque()
    ...
    def handle_websocket_upgrade(self) -> None:
        if self.logger.level <= TRACE_LOG_LEVEL:
            ...
            self.logger.log(TRACE_LOG_LEVEL,
                            "%sUpgrading to WebSocket", prefix)
    ...
    def on_headers_complete(self) -> None:
        ...
        if self.limit_concurrency is not None and (
            len(self.connections) >= self.limit_concurrency or len(
                self.tasks) >= self.limit_concurrency
        ):
    ...
    def _start_asgi_task(self, cycle: RequestResponseCycle, app: ASGI3Application) -> None:
        if self.config.reset_contextvars:
            ...
            if sys.version_info >= (3, 11):  # pragma: py-lt-311
                task = self.loop.create_task(cycle.run_asgi(
                    app), context=contextvars.Context())
...
class RequestResponseCycle:
        ...
        if not self.response_started:
            ...
            for name, value in headers:
                if HEADER_RE.search(name):
                    raise RuntimeError(
                        "Invalid HTTP header name.")  # pragma: full coverage
    ...
    async def receive(self) -> ASGIReceiveEvent:
        ...
        message: HTTPRequestEvent = {"type": "http.request", "body": bytes(
            self.body), "more_body": self.more_body}
        self.body = bytearray()
        return message
