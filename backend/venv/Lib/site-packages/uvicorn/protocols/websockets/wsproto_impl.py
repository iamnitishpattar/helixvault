...
class WebsocketBuffer:
    ...
    def extend(self, event: events.TextMessage | events.BytesMessage) -> None:
        ...
        self.length += len(event.data.encode()) if isinstance(event,
                                                              events.TextMessage) else len(event.data)
...
class WSProtocol(asyncio.Protocol):
    ...
    def connection_made(self, transport: asyncio.Transport) -> None:
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            ...
            self.logger.log(TRACE_LOG_LEVEL,
                            "%sWebSocket connection made", prefix)
    ...
    def connection_lost(self, exc: Exception | None) -> None:
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            prefix = "%s:%d - " % self.client if self.client else ""
            self.logger.log(TRACE_LOG_LEVEL,
                            "%sWebSocket connection lost", prefix)
    ...
    def handle_connect(self, event: events.Request) -> None:
        ...
        full_raw_path = self.root_path.encode(
            "ascii") + raw_path.encode("ascii")
    ...
    def handle_message(self, event: events.TextMessage | events.BytesMessage) -> None:
        try:
            ...
            if not self.transport.is_closing():
                self.transport.write(self.conn.send(
    ...
    def send_keepalive_ping(self) -> None:
        ...
        self.transport.write(self.conn.send(
            wsproto.events.Ping(payload=self.pending_ping_payload)))
    ...
    def keepalive_timeout(self) -> None:
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            prefix = "%s:%d - " % self.client if self.client else ""
            self.logger.log(TRACE_LOG_LEVEL,
                            "%sWebSocket keepalive ping timeout", prefix)
        reason = "keepalive ping timeout"
        self.transport.write(self.conn.send(
            wsproto.events.CloseConnection(code=1011, reason=reason)))
        self.close_sent = True
        self.transport.close()
        ...
        if not self.handshake_complete:
            ...
            try:
                if message["type"] == "websocket.send":
                    ...
                    output = self.conn.send(
                        wsproto.events.Message(data=data))  # type: ignore
                    if not self.transport.is_closing():
                        self.transport.write(output)
