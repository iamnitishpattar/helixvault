    ...
    def set_cookie(
        ...
        if partitioned:
            if sys.version_info < (3, 14):
                raise ValueError(
                    "Partitioned cookies are only supported in Python 3.14 and above.")  # pragma: no cover
...
class FileResponse(Response):
    ...
    def __init__(
        ...
        if media_type is None:
            media_type = guess_type(filename or path)[
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        send_header_only = scope_type == "http" and scope["method"].upper(
        ) == "HEAD"
        ...
        if http_range is None or (http_if_range is not None and not self._should_use_range(http_if_range)):
            ...
            try:
                ...
                response = PlainTextResponse(status_code=416, headers={
                                             "Content-Range": f"bytes */{exc.max_size}"})
        ...
        for part in range_.split(","):
            ...
            try:
                start = int(start_str) if start_str else file_size - \
                    ...
                    end_str) + 1 if start_str and end_str and int(end_str) < file_size else file_size
    ...
    def generate_multipart(
        ...
        static_header_part_len = 49 + boundary_len + \
            len(content_type) + len(str(max_size))
        content_length = sum(
            (len(str(start)) + len(str(end - 1)) +
             static_header_part_len)  # Headers
            + (end - start)  # Content
            for start, end in ranges
        ) + (
