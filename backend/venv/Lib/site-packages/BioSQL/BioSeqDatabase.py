...
def open_database(driver="MySQLdb", **kwargs):
    ...
    if os.name == "java":
        if driver in ["MySQLdb"]:
            ...
            conn = connect(
                url_pref + kw.get("database", "postgresql") +
...
class DBServer:
    ...
    def load_database_sql(self, sql_file):
        ...
        # SQL executed one at a time
            ...
            raise ValueError(
                f"Module {self.module_name} not supported by the loader.")
...
class BioSeqDatabase:
    ...
    def __getitem__(self, key):
        ...
        if record._biodatabase_id != self.dbid:
            raise KeyError(
                f"Entry {key!r} does exist, but not in current name space")
        return record
    ...
    def __contains__(self, value):
        ...
        try:
            ...
            return False
        return bool(
            self.adaptor.execute_and_fetch_col0(
                sql, (self.dbid, bioentry_id))[0]
        )
    ...
    def load(self, record_iterator, fetch_NCBI_taxonomy=False):
        ...
        """
        db_loader = Loader.DatabaseLoader(
            self.adaptor, self.dbid, fetch_NCBI_taxonomy)
        num_records = 0
