...
def _retrieve_seq(adaptor, primary_id):
    ...
    try:
        ...
        if seq:
            raise ValueError(
                f"Expected 'seq' to have a falsy value, got {seq}.")
...
def _retrieve_alphabet(adaptor, primary_id):
    ...
    if len(alphabets) != 1:
        raise ValueError(
            f"Expected 1 alphabet in response, got {len(alphabets)}.")
...
class DBSeqRecord(SeqRecord):
    ...
    seq = property(__get_seq, __set_seq, __del_seq,
                   "Seq object")  # type: ignore
    ...
    def __get_features(self):
        if not hasattr(self, "_features"):
            self._features = _retrieve_features(
                self._adaptor, self._primary_id)
