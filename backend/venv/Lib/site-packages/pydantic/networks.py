    ...
    def __hash__(self) -> int:
        return hash(
            (
                self.max_length,
                tuple(
                    self.allowed_schemes) if self.allowed_schemes is not None else None,
                self.host_required,
                self.default_host,
                self.default_port,
                self.default_path,
...
class _BaseUrl:
    ...
    def __init__(self, url: str | _CoreUrl | _BaseUrl) -> None:
        self._url = _build_type_adapter(
            self.__class__).validate_python(url)._url
    ...
    __pydantic_serializer__ = SchemaSerializer(core_schema.any_schema(
...
class MariaDBDsn(AnyUrl):
    ...
    * Host not required
    ...
    _constraints = UrlConstraints(
        allowed_schemes=['mariadb',
                         'mariadb+mariadbconnector', 'mariadb+pymysql'],
        default_port=3306,
    )
...
def import_email_validator() -> None:
    ...
    try:
        ...
        raise ImportError(
            "email-validator is not installed, run `pip install 'pydantic[email]'`") from e
...
def validate_email(value: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise PydanticCustomError(
            'value_error', 'value is not a valid email address: {reason}', {
                'reason': str(e.args[0])}
        ) from e
