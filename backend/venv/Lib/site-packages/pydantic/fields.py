...
class FieldInfo(_repr.Representation):
    ...
    def __init__(self, **kwargs: Unpack[_FieldInfoInputs]) -> None:
        ...
        self._attributes_set = {k: v for k, v in kwargs.items(
        ) if v is not _Unset and k not in self.metadata_lookup}
        kwargs = {k: _DefaultValues.get(
            # type: ignore
            k) if v is _Unset else v for k, v in kwargs.items()}
        ...
        alias_is_set = any(alias is not None for alias in (
            self.alias, self.validation_alias, self.serialization_alias))
        self.alias_priority = kwargs.pop(
            'alias_priority', None) or 2 if alias_is_set else None
    ...
    def from_annotated_attribute(
        ...
        if isinstance(default, FieldInfo):
            # Copy unnecessary when we remove HACK 1.
            default_copy = default._copy()
            ...
            # Unnecessary when we remove HACK 1.
            ...
            if isinstance(meta, FieldInfo):
                ...
                if meta._final:
                    merged_kwargs.update(
                        {attr: getattr(meta, attr) for attr in _Attrs})
                else:
    ...
    @overload
    def get_default(
        self, *, call_default_factory: Literal[False] = ...) -> Any: ...
...
# type hint the return value as `Any` to avoid type checking regressions when using `...`.
@overload
def Field(
...
# `default` argument set, validate_default=True (no type checking on the default value)
@overload
def Field(
    default: Any,
    *,
...
# `default_factory` argument set, validate_default=True  (no type checking on the default value)
@overload
...
class ComputedFieldInfo:
    ...
    def __copy__(self) -> Self:
        return type(self)(
            ...
            examples=self.examples.copy() if isinstance(
                self.examples, list) else self.examples,
    ...
    def _apply_alias_generator(self, alias_generator: Callable[[str], str] | AliasGenerator, name: str) -> None:
        ...
        if self.alias_priority is None or self.alias_priority <= 1 or self.alias is None:
            ...
            if isinstance(alias_generator, AliasGenerator):
                alias, _, serialization_alias = alias_generator.generate_aliases(
                    name)
            ...
            if self.alias_priority == 1:
                self.alias = _utils.get_first_not_none(
                    serialization_alias, alias)
...
def computed_field(
    *,
    ...
    field_title_generator: Callable[[
        str, ComputedFieldInfo], str] | None = None,
