...
HslColorTuple = Union[tuple[float, float, float],
...
class RGBA:
    ...
    def __init__(self, r: float, g: float, b: float, alpha: Optional[float]):
        ...
        self._tuple: tuple[float, float, float,
                           Optional[float]] = (r, g, b, alpha)
        ...
        if self._rgba.alpha is None:
            ...
            try:
                ...
                if fallback:
                    ...
                    raise ValueError(
                        'no named color found, use fallback=True, as_hex() or as_rgb()') from e
        else:
            return self.as_hex()
...
def parse_float_alpha(value: Union[None, str, float, int]) -> Optional[float]:
    ...
    if math.isclose(alpha, 1):
        ...
        raise PydanticCustomError(
            'color_error', 'value is not a valid color: alpha values must be in the range 0 to 1')
...
def parse_hsl(h: str, h_units: str, sat: str, light: str, alpha: Optional[float] = None) -> RGBA:
    ...
    Returns:
        An instance of `RGBA`.
    """
    s_value, l_value = parse_color_value(
        sat, 100), parse_color_value(light, 100)
