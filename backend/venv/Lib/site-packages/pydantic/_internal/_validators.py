    ...
    * the substring of `dotted_path` after the colon is not an attribute of the module (e.g., 'collections:abc123')
    ...
    if attribute is not None:
        try:
            ...
            raise ImportError(
                f'cannot import name {attribute!r} from {module_path!r}') from e
    else:
        return module
...
def fraction_validator(input_value: Any, /) -> Fraction:
    ...
    try:
        ...
        raise PydanticCustomError(
            'fraction_parsing', 'Input is not a valid fraction')
...
def greater_than_validator(x: Any, gt: Any) -> Any:
    try:
        ...
        raise TypeError(
            f"Unable to apply constraint 'gt' to supplied value {x}")
...
def greater_than_or_equal_validator(x: Any, ge: Any) -> Any:
    try:
        ...
        raise TypeError(
            f"Unable to apply constraint 'ge' to supplied value {x}")
...
def less_than_validator(x: Any, lt: Any) -> Any:
    try:
        ...
        raise TypeError(
            f"Unable to apply constraint 'lt' to supplied value {x}")
...
def less_than_or_equal_validator(x: Any, le: Any) -> Any:
    try:
        ...
        raise TypeError(
            f"Unable to apply constraint 'le' to supplied value {x}")
...
def max_length_validator(x: Any, max_length: Any) -> Any:
    try:
        if len(x) > max_length:
            raise PydanticKnownError(
                'too_long',
                {'field_type': 'Value', 'max_length': max_length,
                    'actual_length': len(x)},
            )
        return x
    except TypeError:
...
def decimal_places_validator(x: Any, decimal_places: Any) -> Any:
    try:
        ...
        if decimal_places_ > decimal_places:
            normalized_decimal_places, _ = _extract_decimal_digits_info(
                x.normalize())
...
def get_defaultdict_default_default_factory(values_source_type: Any) -> Callable[[], Any]:
    ...
    if typing_objects.is_annotated(values_type_origin):
        field_info = next((v for v in get_args(
            values_source_type) if isinstance(v, FieldInfo)), None)
    else:
        field_info = None
    if field_info and field_info.default_factory:
        # Assume the default factory does not take any argument:
        default_default_factory = cast(
...
IpType = Union[IPv4Address, IPv6Address, IPv4Network,
