...
def _apply_alias_generator_to_field_info(
    ...
    if (
        ...
        if field_info.alias_priority == 1:
            ...
            field_info.validation_alias = get_first_not_none(
...
_deprecated_method_names = {'dict', 'json', 'copy',
                            '_iter', '_copy_and_set_values', '_calculate_keys'}
...
def collect_model_fields(  # noqa: C901
    ...
    *,
    ...
    type_hints = _typing_extra.get_model_type_hints(
        cls, ns_resolver=ns_resolver)
    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
        ...
        if assigned_value is not PydanticUndefined and (
            ...
            any(getattr(BaseModel_, depr_name, None)
                is assigned_value for depr_name in _deprecated_method_names)
            # One of the deprecated class methods was used as a field name (e.g. `schema()`):
            or (
                hasattr(assigned_value, '__func__')
                and any(
                    # pyright: ignore[reportAttributeAccessIssue]
                    getattr(getattr(BaseModel_, depr_name, None),
                            '__func__', None) is assigned_value.__func__
        ...
        # An assigned value is present (either the default value, or a `Field()` function)
        else:
    ...
    *,
    ...
    if lenient:
        ann = _generics.replace_types(
            field_info._original_annotation, typevars_map)
