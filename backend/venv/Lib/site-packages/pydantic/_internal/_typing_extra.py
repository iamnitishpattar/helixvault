...
_NONE_TYPES: tuple[Any, ...] = (
    None, NoneType, typing.Literal[None], typing_extensions.Literal[None])
...
if sys.version_info < (3, 10):
    ...
    WithArgsTypes: tuple[Any, ...] = (
        typing._GenericAlias, types.GenericAlias, types.UnionType)
            ...
            for name, value in ann.items():
                if name.startswith('_'):
                    ...
                    if base_model_fields is not None and name in base_model_fields:
                        ...
                        hints[name] = (
                            base_model_fields[name].annotation, True)
                    else:
                        globalns, localns = ns_resolver.types_namespace
...
def _eval_type_backport(
    ...
    try:
        ...
        return eval_type_backport(
            value,
            globalns,
            # pyright: ignore[reportArgumentType], waiting on a new `eval_type_backport` release.
            localns,
            try_default=False,
        )
    ...
    def get_type_hints(  # noqa: C901
        ...
        * Use `_make_forward_ref` instead of `typing.ForwardRef` to handle the `is_class` argument.
        ...
        if isinstance(obj, type):
            ...
            for base in reversed(obj.__mro__):
                if globalns is None:
                    base_globals = getattr(sys.modules.get(
                        base.__module__, None), '__dict__', {})
                else:
