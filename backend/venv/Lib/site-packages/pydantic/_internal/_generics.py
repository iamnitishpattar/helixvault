...
class PydanticGenericMetadata(TypedDict):
    # analogous to typing._GenericAlias.__origin__
...
def _get_caller_frame_info(depth: int = 2) -> tuple[str | None, bool]:
    ...
    try:
        ...
        raise RuntimeError(
            'This function must be used inside another function') from e
...
def replace_types(type_: Any, type_map: Mapping[TypeVar, Any] | None) -> Any:
    ...
    if type_args:
        resolved_type_args = tuple(replace_types(
            arg, type_map) for arg in type_args)
        ...
        if (3, 10) <= sys.version_info < (3, 14) and origin_type is types.UnionType:
    ...
    if not origin_type and is_model_class(type_):
        ...
        if not parameters:
            return type_
        resolved_type_args = tuple(replace_types(t, type_map)
                                   for t in parameters)
    ...
    for parameter, argument in zip_longest(parameters, args, fillvalue=_missing):
        ...
        if argument is _missing:
            ...
            if has_default:
                ...
                # pyright: ignore[reportAttributeAccessIssue]
                expected_len -= sum(hasattr(p, 'has_default')
                                    and p.has_default() for p in parameters)
                raise TypeError(
...
def set_cached_generic_type(
    ...
    if len(typevar_values) == 1:
        _GENERIC_TYPES_CACHE[_early_cache_key(
            parent, typevar_values[0])] = type_
    if origin and args:
        _GENERIC_TYPES_CACHE[_late_cache_key(
            origin, args, typevar_values)] = type_
