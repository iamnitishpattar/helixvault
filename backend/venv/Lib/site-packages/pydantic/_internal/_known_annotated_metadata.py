...
SEQUENCE_SCHEMA_TYPES = ('list', 'tuple', 'set',
                         'frozenset', 'generator', *TEXT_SCHEMA_TYPES)
NUMERIC_SCHEMA_TYPES = ('float', 'int', 'date', 'time',
                        'timedelta', 'datetime')
...
constraint_schema_pairings: list[tuple[set[str], tuple[str, ...]]] = [
    ...
    (STRICT, (*TEXT_SCHEMA_TYPES, *SEQUENCE_SCHEMA_TYPES,
     *NUMERIC_SCHEMA_TYPES, 'typed-dict', 'model')),
        ...
        if constraint in chain_schema_constraints:
            ...
            chain_schema_steps.append(
                cs.no_info_wrap_validator_function(
                    _apply_constraint_with_incompatibility_info, cs.str_schema(
                        **{constraint: value})
                )
            )
            ...
            schema = cs.no_info_after_validator_function(
                partial(
                    NUMERIC_VALIDATOR_LOOKUP[constraint], **{constraint: value}), schema
            )
    ...
    for annotation in other_metadata:
        if (annotation_type := type(annotation)) in (at_to_constraint_map := _get_at_to_constraint_map()):
            ...
            if validator is None:
                raise ValueError(f'Unknown constraint {constraint}')
            schema = cs.no_info_after_validator_function(
                partial(validator, {constraint: getattr(
                    annotation, constraint)}), schema
            )
            continue
        elif isinstance(annotation, (at.Predicate, at.Not)):
            predicate_name = f'{annotation.func.__qualname__!r} ' if hasattr(
                annotation.func, '__qualname__') else ''
...
def collect_known_metadata(annotations: Iterable[Any]) -> tuple[dict[str, Any], list[Any]]:
    ...
    for annotation in annotations:
        ...
        # we don't use dataclasses.asdict because that recursively calls asdict on the field values
            ...
            res.update({k: v for k, v in vars(
                annotation).items() if not k.startswith('_')})
        else:
