        ...
        for base in reversed(mro(typ)[1:-1]):
            ...
            if existing is None:
                existing, _ = _decorator_infos_for_class(
                    base, collect_to_replace=False)
            res.validators.update({k: v.bind_to_cls(typ)
...
def inspect_validator(
    ...
    if mode == 'wrap':
        ...
        assert mode in {
            'before', 'after', 'plain'}, f"invalid mode: {mode!r}, expected 'before', 'after' or 'plain"
        if n_positional == 2:
            return True
        elif n_positional == 1:
            return False
...
def ensure_classmethod_based_on_signature(function: AnyDecoratorCallable) -> Any:
    ...
    if not isinstance(
        unwrap_wrapped_function(
            function, unwrap_class_static_method=False), classmethod
