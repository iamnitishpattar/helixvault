...
class PydanticPlugin(Plugin):
    ...
    def _pydantic_create_model_callback(self, ctx: DynamicClassDefContext) -> None:
        ...
        if base_sym is None or not isinstance(base_sym.node, TypeInfo):
            ...
            base_sym = ctx.api.lookup_fully_qualified_or_none(
                BASEMODEL_FULLNAME)
            if base_sym is None or not isinstance(base_sym.node, TypeInfo):
                return
        ...
        info = ctx.api.basic_new_typeinfo(
            ctx.name, base_instance, ctx.call.line)
...
def from_attributes_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx_type, CallableType) and isinstance(ctx_type.ret_type, Instance):
        ...
        # called on an instance (unusual, but still valid)
        model_type = ctx_type
    ...
    if not model_type.type.has_base(BASEMODEL_FULLNAME):
        ...
        return ctx.default_return_type
    from_attributes = pydantic_metadata.get(
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        self.add_model_construct_method(
            fields, config, is_settings, is_a_root_model)
            ...
            if isinstance(sym.node, Decorator):
                ...
                if (
                    ...
                    and not (
                        ...
                        and any(
                            first_dec.arg_names[i] == 'mode' and isinstance(
                                arg, StrExpr) and arg.value == 'after'
            ...
            if isinstance(stmt, AssignmentStmt):
                ...
                if isinstance(stmt.rvalue, CallExpr):  # calls to `dict` or `ConfigDict`
                    for arg_name, arg in zip(stmt.rvalue.arg_names, stmt.rvalue.args):
                        if arg_name is None:
                            continue
                        config.update(self.get_config_update(
    ...
    def add_initializer(
        ...
        use_alias = not (
            config.validate_by_name or config.populate_by_name) and config.validate_by_alias is not False
        ...
        if not self.should_init_forbid_extra(fields, config):
            var = Var('kwargs')
            args.append(Argument(var, AnyType(
                TypeOfAny.explicit), None, ARG_STAR2))

        add_method(self._api, self._cls, '__init__',
                   args=args, return_type=NoneType())
    ...
    def add_model_construct_method(
        ...
        args = args + \
            [fields_set_argument] if is_root_model else [
    ...
    def get_alias_info(stmt: AssignmentStmt) -> tuple[str | None, bool]:
        ...
        if not (
            isinstance(expr, CallExpr) and isinstance(
                expr.callee, RefExpr) and expr.callee.fullname == FIELD_FULLNAME
        ):
...
def error_from_attributes(model_name: str, api: CheckerPluginInterface, context: Context) -> None:
    ...
    api.fail(f'"{model_name}" does not have from_attributes=True',
             context, code=ERROR_ORM)
...
def error_extra_fields_on_root_model(api: CheckerPluginInterface, context: Context) -> None:
    ...
    api.fail('Only `root` is allowed as a field of a `RootModel`',
             context, code=ERROR_EXTRA_FIELD_ROOT_MODEL)
...
def add_method(
    ...
    signature = CallableType(
        arg_types, arg_kinds, arg_names, return_type, function_type, variables=[
            tvar_def] if tvar_def else None
    )
...
def parse_toml(config_file: str) -> dict[str, Any] | None:
    ...
    if sys.version_info >= (3, 11):
        ...
        try:
            ...
            warnings.warn(
                'No TOML parser installed, cannot read configuration from `pyproject.toml`.', stacklevel=2)
            return None
