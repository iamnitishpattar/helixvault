...
JsonValue: TypeAlias = Union[int, float, str,
                             bool, None, list['JsonValue'], 'JsonDict']
...
@overload
def with_config(**config: Unpack[ConfigDict]
...
def with_config(config: ConfigDict | None = None, /, **kwargs: Any) -> Callable[[_TypeT], _TypeT]:
    ...
    if len(kwargs) == 1 and (kwargs_conf := kwargs.get('config')) is not None:
        ...
        final_config = config if config is not None else cast(
            ConfigDict, kwargs)
