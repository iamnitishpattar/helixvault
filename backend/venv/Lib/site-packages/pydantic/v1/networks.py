...
NetworkType = Union[str, bytes, int,
                    Tuple[Union[str, bytes, int], Union[str, int]]]
...
def int_domain_regex() -> Pattern[str]:
    ...
    if _int_domain_regex_cache is None:
        ...
        _int_domain_regex_cache = re.compile(
            fr'(?:{int_chunk}\.)*?{int_chunk}{int_domain_ending}', re.IGNORECASE)
...
class AnyUrl(str):
    ...
    __slots__ = ('scheme', 'user', 'password', 'host', 'tld',
                 'host_type', 'port', 'path', 'query', 'fragment')
    ...
    def __modify_schema__(cls, field_schema: Dict[str, Any]) -> None:
        update_not_none(field_schema, minLength=cls.min_length,
                        maxLength=cls.max_length, format='uri')
    ...
    def __repr__(self) -> str:
        extra = ', '.join(f'{n}={getattr(self, n)!r}' for n in self.__slots__ if getattr(
            self, n) is not None)
