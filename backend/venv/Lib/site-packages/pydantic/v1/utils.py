...
if TYPE_CHECKING:
    ...
    RichReprResult = Iterable[Union[Any, Tuple[Any],
...
def smart_deepcopy(obj: Obj) -> Obj:
    ...
    try:
        if not obj and obj_type in BUILTIN_COLLECTIONS:
            ...
            # type: ignore  # tuple doesn't have copy method
...
def get_discriminator_alias_and_values(tp: Any, discriminator_key: str) -> Tuple[str, Tuple[str, ...]]:
    ...
    if is_union(get_origin(tp)):
        ...
        try:
            ...
            raise TypeError(
                f'Type {tp.__name__!r} is not a valid `BaseModel` or `dataclass`') from e
        except KeyError as e:
            raise ConfigError(
                f'Model {tp.__name__!r} needs a discriminator field for key {discriminator_key!r}') from e
...
def _get_union_alias_and_all_values(
    ...
    zipped_aliases_values = [get_discriminator_alias_and_values(
        t, discriminator_key) for t in get_args(union_type)]
