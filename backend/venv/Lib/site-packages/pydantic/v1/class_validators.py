    ...
    *fields: str,
    ...
    def dec(f: AnyCallable) -> 'AnyClassMethod':
        ...
        setattr(
            ...
            (
                fields,
                Validator(func=f_cls.__func__, pre=pre, each_item=each_item,
                          always=always, check_fields=check_fields),
            ),
        )
        return f_cls
...
def root_validator(
    ...
    if _func:
        ...
        setattr(
            f_cls, ROOT_VALIDATOR_CONFIG_KEY, Validator(
...
def _prepare_validator(function: AnyCallable, allow_reuse: bool) -> 'AnyClassMethod':
    ...
    """
    f_cls = function if isinstance(
        function, classmethod) else classmethod(function)
    if not in_ipython() and not allow_reuse:
        ref = (
            getattr(f_cls.__func__, '__module__', '<No __module__>')
            + '.'
            + getattr(f_cls.__func__, '__qualname__',
...
class ValidatorGroup:
    ...
    def check_for_unused(self) -> None:
        unused_validators = set(
            chain.from_iterable(
                (
                    getattr(v.func, '__name__',
                            f'<No __name__: id:{id(v.func)}>')
