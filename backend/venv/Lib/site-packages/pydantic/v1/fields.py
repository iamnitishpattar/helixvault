...
MAPPING_LIKE_SHAPES: Set[int] = {
    SHAPE_DEFAULTDICT, SHAPE_DICT, SHAPE_MAPPING, SHAPE_COUNTER}
...
class ModelField(Representation):
    ...
    def _get_field_info(
        ...
        if get_origin(annotation) is Annotated:
            field_infos = [arg for arg in get_args(
                annotation)[1:] if isinstance(arg, FieldInfo)]
    ...
    def _type_analysis(self) -> None:  # noqa: C901 (ignore complexity)
        ...
        if origin is None or origin is CollectionsHashable:
            ...
            if len(types_) == 1:
                ...
                self.sub_fields = [self._create_sub_type(
                    t, f'{self.name}_{display_as_type(t)}') for t in types_]
            ...
            if not args:  # plain tuple
                ...
                self.shape = SHAPE_TUPLE
                self.sub_fields = [self._create_sub_type(
                    t, f'{self.name}_{i}') for i, t in enumerate(args)]
            return
            ...
            if get_validators:
                self.class_validators.update(
                    {f'frozenset_{i}': Validator(
                        validator, pre=True) for i, validator in enumerate(get_validators())}
                )
        ...
        # check for Iterable and CollectionsIterable, as it could receive one even when declared with the other
            ...
            self.sub_fields = [self._create_sub_type(
                t, f'{self.name}_{i}') for i, t in enumerate(get_args(self.type_))]
            self.type_ = origin
            return
        else:
    ...
    def populate_validators(self) -> None:
        ...
        if not self.sub_fields or self.shape == SHAPE_GENERIC:
            ...
            v_funcs = (
                ...
                *(get_validators() if get_validators else list(
                    find_validators(self.type_, self.model_config))),
    ...
    def _validate_sequence_like(  # noqa: C901 (ignore complexity)
        ...
        converted: Union[List[Any], Set[Any], FrozenSet[Any],
                         Tuple[Any, ...], Iterator[Any], Deque[Any]] = result
    ...
    def _validate_tuple(
        ...
        if not sequence_like(v):
            ...
            actual_length, expected_length = len(
                v), len(self.sub_fields)  # type: ignore
    ...
    def _validate_mapping_like(
        ...
        for k, v_ in v_iter.items():
            v_loc = *loc, '__key__'
            key_result, key_errors = self.key_field.validate(
                k, values, loc=v_loc, cls=cls)  # type: ignore
            if key_errors:
    ...
    def _get_mapping_value(self, original: T, converted: Dict[Any, Any]) -> Union[T, Dict[Any, Any]]:
        ...
        if original_cls == dict or original_cls == Dict:
            ...
            try:
                ...
                raise RuntimeError(
                    f'Could not convert dictionary to {original_cls.__name__!r}') from None
    ...
    def _type_display(self) -> PyObjectStr:
        ...
        if self.shape in MAPPING_LIKE_SHAPES:
            ...
            t = 'Tuple[{}]'.format(', '.join(display_as_type(
                f.type_) for f in self.sub_fields))  # type: ignore
        elif self.shape == SHAPE_GENERIC:
            assert self.sub_fields
            t = '{}[{}]'.format(
                display_as_type(self.type_), ', '.join(
                    display_as_type(f.type_) for f in self.sub_fields)
            )
