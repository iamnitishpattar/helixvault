...
BUILTINS_NAME = 'builtins' if MYPY_VERSION_TUPLE >= (
    0, 930) else '__builtins__'
        ...
        if default_args:
            ...
            if isinstance(default_factory_type, Overloaded):
                if MYPY_VERSION_TUPLE > (0, 910):
                    ...
                    default_factory_type = default_factory_type.items()[
...
class PydanticModelTransformer:
    ...
    def transform(self) -> None:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        is_settings = any(get_fullname(base) ==
                          BASESETTINGS_FULLNAME for base in info.mro[:-1])
    ...
    def add_construct_method(self, fields: List['PydanticModelField']) -> None:
        ...
        """
        ctx = self._ctx
        set_str = ctx.api.named_type(f'{BUILTINS_NAME}.set', [
        ...
        # Make sure it does not conflict with other names in the class
...
def error_default_and_default_factory_specified(api: CheckerPluginInterface, context: Context) -> None:
    api.fail('Field default and default_factory cannot be specified together',
             context, code=ERROR_FIELD_DEFAULTS)
...
def add_method(
    ...
    if is_classmethod or is_new:
        first = [Argument(Var('_cls'), TypeType.make_normalized(
            self_type), None, ARG_POS)]
