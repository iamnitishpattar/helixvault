...
class BaseSettings(BaseModel):
    ...
    def _build_values(
        ...
        env_settings = EnvSettingsSource(
            env_file=(_env_file if _env_file !=
                      env_file_sentinel else self.__config__.env_file),
            env_file_encoding=(
...
class EnvSettingsSource:
    __slots__ = ('env_file', 'env_file_encoding',
                 'env_nested_delimiter', 'env_prefix_len')
    ...
    def _read_env_files(self, case_sensitive: bool) -> Dict[str, Optional[str]]:
        ...
        for env_file in env_files:
            ...
            if env_path.is_file():
                dotenv_vars.update(
                    read_env_file(
                        env_path, encoding=self.env_file_encoding, case_sensitive=case_sensitive)
                )
...
def read_env_file(
    ...
    try:
        ...
        raise ImportError(
            'python-dotenv is not installed, run `pip install pydantic[dotenv]`') from e
