...
class ModelMetaclass(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):  # noqa C901
        ...
        config_kwargs = {key: kwargs.pop(
            key) for key in kwargs.keys() & allowed_config_kwargs}
        ...
        if (namespace.get('__module__'), namespace.get('__qualname__')) != ('pydantic.main', 'BaseModel'):
            ...
            for ann_name, ann_type in annotations.items():
                if is_classvar(ann_type):
                    ...
                    allowed_types = get_args(ann_type) if is_union(
                        get_origin(ann_type)) else (ann_type,)
                    if (
                        is_untouched(value)
            ...
            for var_name, value in namespace.items():
                can_be_changed = var_name not in class_vars and not is_untouched(
                    value)
...
class BaseModel(Representation, metaclass=ModelMetaclass):
    ...
    def __getstate__(self) -> 'DictAny':
        private_attrs = ((k, getattr(self, k, Undefined))
    ...
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        ...
        if not isinstance(obj, dict):
            try:
                ...
                raise ValidationError(
                    [ErrorWrapper(exc, loc=ROOT_KEY)], cls) from e
        return cls(**obj)
    ...
    def from_orm(cls: Type['Model'], obj: Any) -> 'Model':
        ...
        obj = {ROOT_KEY: obj} if cls.__custom_root_type__ else cls._decompose_class(
            obj)
        m = cls.__new__(cls)
    ...
    def copy(
        ...
        *,
        ...
        values = dict(
            self._iter(to_dict=False, by_alias=False, include=include,
                       exclude=exclude, exclude_unset=False),
            **(update or {}),
        )
    ...
    def _iter(
        ...
        value_exclude = ValueItems(
            self, exclude) if exclude is not None else None
        value_include = ValueItems(
            self, include) if include is not None else None
        ...
        for field_key, v in self.__dict__.items():
            ...
            if to_dict or value_include or value_exclude:
                v = self._get_value(
                    ...
                    to_dict=to_dict,
                    by_alias=by_alias,
                    include=value_include and value_include.for_element(
                        field_key),
                    exclude=value_exclude and value_exclude.for_element(
                        field_key),
                    exclude_unset=exclude_unset,
                    exclude_defaults=exclude_defaults,
                    exclude_none=exclude_none,
