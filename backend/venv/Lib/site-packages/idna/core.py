...
def uts46_remap(domain: str, std3_rules: bool = True, transitional: bool = False) -> str:
    ...
    for pos, char in enumerate(domain):
        code_point = ord(char)
        i = code_point if code_point < 256 else bisect.bisect_right(
            uts46_starts, code_point) - 1
        ...
        keep_as_is = (
            status == "V" or (status == "D" and not transitional) or (
        ...
        use_replacement = replacement is not None and (
            status == "M" or (status == "3" and not std3_rules) or (
...
def encode(
    ...
    if not isinstance(s, str):
        try:
            ...
            raise IDNAError(
                "should pass a unicode string to the function rather than a byte string.") from err
