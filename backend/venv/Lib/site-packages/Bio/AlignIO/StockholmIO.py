...
class StockholmWriter(SequentialAlignmentWriter):
    ...
    pfam_gs_mapping = {"organism": "OS",
                       "organism_classification": "OC", "look": "LO"}
        ...
        if alignment.column_annotations:
            for k, v in sorted(alignment.column_annotations.items()):
                if k in self.pfam_gc_mapping:
                    ...
                    self.handle.write(
                        f"#=GC {self.pfam_gr_mapping[k]}_cons {v}\n")
                else:
    ...
    def _write_record(self, record):
        ...
        if record.description:
            self.handle.write(
                f"#=GS {seq_name} DE {self.clean(record.description)}\n")
...
class StockholmIterator(AlignmentIterator):
    ...
    pfam_gs_mapping = {"OS": "organism",
                       "OC": "organism_classification", "LO": "look"}
    ...
    def _populate_meta_data(self, identifier, record):
        ...
        for feature in seq_data:
            ...
            if feature == "AC":  # ACcession number
                ...
                # Ignore it?
                record.annotations["GS:" +
                                   feature] = ", ".join(seq_data[feature])
        ...
        for feature in seq_col_data:
            ...
            if feature in self.pfam_gr_mapping:
                ...
                # Ignore it?
                record.letter_annotations["GR:" +
                                          feature] = seq_col_data[feature]
