...
def MafIterator(handle, seq_count=None):
    ...
    while True:
        ...
        if in_a_bundle:
            ...
            annotations = dict(a_string.split("=")
                               for a_string in annot_strings)
        elif line.startswith("#"):
            # ignore comments
            pass
        elif not line:
...
class MafIndex:
    ...
    def __make_new_index(self):
        ...
        self._con.execute(
            "CREATE INDEX IF NOT EXISTS bin_index ON offset_data(bin);")
        self._con.execute(
            "CREATE INDEX IF NOT EXISTS start_index ON offset_data(start);"
        )
        self._con.execute(
            "CREATE INDEX IF NOT EXISTS end_index ON offset_data(end);")
    ...
    def search(self, starts, ends):
        ...
        *ends* should be the list of the corresponding segment ends
        ...
        if len(starts) != len(ends):
            raise ValueError(
                "Every position in starts must have a match in ends")
