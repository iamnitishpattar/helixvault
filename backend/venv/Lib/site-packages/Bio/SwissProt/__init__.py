...
def _read(handle):
    ...
    if key != "ID":
        raise SwissProtParserError(
            "Failed to find ID in first line", line=line)
    record = Record()
    ...
    for line in handle:
        ...
        # an author.  Each block must have at least 1 RA or RG line.
            ...
            raise SwissProtParserError(
                f"Unknown keyword {key!r} found", line=line)
    if record:
...
def _read_id(record, line):
    ...
    if len(cols) == 5:
        ...
        raise SwissProtParserError(
            "ID line has unrecognised format", line=line)
...
def _read_dt(record, line):
    ...
    if (
        ...
        if "CREATED" in uprline:
            ...
            raise SwissProtParserError(
                "Unrecognised DT (DaTe) line", line=line)
    elif (
...
def _read_rx(reference, value):
    ...
    # otherwise we assume we have the type 'RX   MEDLINE; 85132727.'
        ...
        if len(cols) != 2:
            ...
            reference.references.append(
                (cols[0].rstrip(";"), cols[1].rstrip(".")))
    if warn:
        import warnings
        ...
        warnings.warn(
            f"Possibly corrupt RX line {value!r}", BiopythonParserWarning)
