...
class BaseXMLWriter(ABC):
    ...
    def write(self, records):
        ...
        reference = html.escape(records.reference).encode(
            "ASCII", "htmlentityreplace")
...
"""
    ...
    def _write_program(self, program):
        self.stream.write(
            b"<BlastOutput_program>%b</BlastOutput_program>\n" % program)

    def _write_version(self, version):
        self.stream.write(
            b"<BlastOutput_version>%b</BlastOutput_version>\n" % version)
...
"""
    ...
    def _write_iteration_num(self, num):
        self.stream.write(
            b"  <Iteration_iter-num>%d</Iteration_iter-num>\n" % num)

    def _write_iteration_query_id(self, query_id):
        self.stream.write(
            b"<Iteration_query-ID>%b</Iteration_query-ID>\n" % query_id)
    ...
    def _write_parameters_matrix(self, value):
        self.stream.write(
            b"      <Parameters_matrix>%b</Parameters_matrix>\n" % value)

    def _write_parameters_expect(self, value):
        self.stream.write(
            b"      <Parameters_expect>%g</Parameters_expect>\n" % value)
    ...
    def _write_parameters_filter(self, value):
        self.stream.write(
            b"      <Parameters_filter>%b</Parameters_filter>\n" % value)
    ...
    def _write_statistics_kappa(self, value):
        self.stream.write(
            b"      <Statistics_kappa>%r</Statistics_kappa>\n" % value)

    def _write_statistics_lambda(self, value):
        self.stream.write(
            b"      <Statistics_lambda>%r</Statistics_lambda>\n" % value)
    ...
    def _write_hit_accession(self, hit_accession):
        self.stream.write(
            b"  <Hit_accession>%b</Hit_accession>\n" % hit_accession)
    ...
    def _write_hsp_bit_score(self, bit_score):
        self.stream.write(
            b"    <Hsp_bit-score>%b</Hsp_bit-score>\n" % bit_score)
    ...
    def _write_hsp_query_from(self, query_from):
        self.stream.write(
            b"     <Hsp_query-from>%d</Hsp_query-from>\n" % query_from)
    ...
    def _write_hsp_pattern_to(self, pattern_to):
        self.stream.write(
            b"     <Hsp_pattern-to>%d</Hsp_pattern-to>\n" % pattern_to)

    def _write_hsp_query_frame(self, query_frame):
        self.stream.write(
            b"     <Hsp_query-frame>%d</Hsp_query-frame>\n" % query_frame)

    def _write_hsp_hit_frame(self, hit_frame):
        self.stream.write(
            b"     <Hsp_hit-frame>%d</Hsp_hit-frame>\n" % hit_frame)
    ...
    def _write_hsp_align_len(self, align_len):
        self.stream.write(
            b"     <Hsp_align-len>%d</Hsp_align-len>\n" % align_len)
