...
def __read_metadata(record, xml_tree):
    ...
    record.database = xml_tree.find(
        "sequence_dbs").find("sequence_db").get("source")
...
def __make_diagram(record, sequence_tree):
    ...
    for seg_tree in sequence_tree.findall("seg"):
        for hit_ele in seg_tree.findall("hit"):
            ...
            if not hit_eles:
                ...
                gap = hit_pos - \
                    int(hit_eles[-1].get("pos")) - hit_motifs[-1].length
            gaps.append(gap)
    ...
    tail_length = (
        sequence_length -
        int(hit_eles[-1].get("pos")) - hit_motifs[-1].length + 1
    )
