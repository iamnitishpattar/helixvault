...
class GenBankWriter(_InsdcWriter):
    ...
    def _write_single_line(self, tag, text):
        ...
        if len(text) > self.MAX_WIDTH - self.HEADER_WIDTH:
            if tag:
                ...
                warnings.warn(
                    f"Annotation {text!r} too long", BiopythonWarning)
        self.handle.write(
    ...
    def _get_date(record):
        ...
        try:
            datetime(int(date[-4:]),
                     months.index(date[3:6]) + 1, int(date[0:2]))
        except ValueError:
            warnings.warn(
    ...
    def _write_the_first_line(self, record):
        ...
        if mol_type and len(mol_type) > 7:
            ...
            mol_type = mol_type.replace(
                "unassigned ", "").replace("genomic ", "")
            if len(mol_type) > 7:
                warnings.warn(
                    f"Molecule type {mol_type!r} too long", BiopythonWarning)
                mol_type = "DNA"
    ...
    def _write_sequence(self, record):
        ...
        for line_number in range(0, seq_len, self.LETTERS_PER_LINE):
            ...
            for words in range(
                line_number, min(
                    line_number + self.LETTERS_PER_LINE, seq_len), 10
            ):
    ...
    def write_record(self, record):
        ...
        self._write_multi_line(
            "SOURCE", self._get_annotation_str(record, "source"))
...
class EmblWriter(_InsdcWriter):
    ...
    def _write_the_first_lines(self, record):
        ...
        if ";" in accession:
            raise ValueError(
                f"Cannot have semi-colon in EMBL accession, '{accession}'")
        if " " in accession:
            # This is out of practicality... might it be allowed?
            raise ValueError(
                f"Cannot have spaces in EMBL accession, '{accession}'")
        ...
        if mol_type not in (
            ...
            warnings.warn(
                f"Non-standard molecule type: {mol_type}", BiopythonWarning)
        ...
        if "DNA" in mol_type_upper:
            ...
            raise ValueError(
                f"failed to understand molecule_type '{mol_type}'")
