...
def xGC_skew(seq, window=1000, zoom=100, r=300, px=100, py=100):
    ...
    ty = Y0
    canvas.create_text(X0, ty, text="%s...%s (%d nt)" %
                       (seq[:7], seq[-7:], len(seq)))
    ty += 20
...
def seq3(seq, custom_map=None, undef_code="Xaa"):
    ...
    threecode = dict(
        list(IUPACData.protein_letters_1to3_extended.items()) +
...
def seq1(seq, custom_map=None, undef_code="X"):
    ...
    onecode = {k.upper(): v for k,
               v in IUPACData.protein_letters_3to1_extended.items()}
...
def six_frame_translations(seq, genetic_code=1):
    ...
    for i in range(3):
        ...
        frames[-(i + 1)] = translate(anti[i: i +
                                          fragment_length], genetic_code)[::-1]
...
class CodonAdaptationIndex(dict):
    ...
    def __init__(self, sequences, table=standard_dna_table):
        ...
        counts = {
            c1 + c2 + c3: 0 for c1 in "ACGT" for c2 in "ACGT" for c3 in "ACGT"}
        ...
        for i in range(0, len(sequence), 3):
            ...
            try:
                ...
                if codon in ["TGA", "TAA", "TAG"]:
                    ...
                    continue
                raise TypeError(
                    f"illegal codon in sequence: {codon}") from None
            else:
                cai_length += 1
