...
class CircularDrawer(AbstractDrawer):
    ...
    def set_track_heights(self):
        ...
        for track in trackunits:
            top = trackunits[track][1] * \
                trackunit_height - track_crop + track_core
            btm = trackunits[track][0] * \
                trackunit_height + track_crop + track_core
            ctr = btm + (top - btm) / 2.0
    ...
    def draw_feature(self, feature):
        ...
        for locstart, locend in feature.locations:
            ...
            feature_sigil, label = self.get_feature_sigil(
                feature, locstart, locend)
    ...
    def draw_bar_graph(self, graph):
        ...
        for pos0, pos1, val in data:
            ...
            # Draw bar
            bar_elements.append(
                self._draw_arc(ctr, ctr + barval, pos0angle,
                               pos1angle, barcolor)
            )
        return bar_elements
    ...
    def draw_heat_graph(self, graph):
        ...
        for pos0, pos1, val in data:
            ...
            heat_elements.append(
                self._draw_arc(btm, top, pos0angle,
                               pos1angle, heat, border=heat)
            )
    ...
    def draw_scale(self, track):
        ...
        if track.start is not None or track.end is not None:
            ...
            x0, y0 = self.xcenter + btm * \
                sin(alpha), self.ycenter + btm * cos(alpha)
            x1, y1 = self.xcenter + top * \
                sin(alpha), self.ycenter + top * cos(alpha)
            scale_elements.append(
                Line(x0, y0, x1, y1, strokeColor=track.scale_color))
        else:
            # Draw a full circle
        ...
        if track.scale_ticks:  # Ticks are required on the scale
            ...
            for tickpos in range(
                tickiterval *
                    (self.start // tickiterval), int(self.end), tickiterval
            ):
    ...
    def draw_greytrack(self, track):
        ...
        if track.start is not None or track.end is not None:
            ...
            greytrack_bgs.append(
                self._draw_arc(
                    btm, top, startangle, endangle, colors.Color(
                        0.96, 0.96, 0.96)
                )
            )
        elif self.sweep < 1:
            ...
            greytrack_bgs.append(
                self._draw_arc(
                    btm, top, 0, 2 * pi *
                    self.sweep, colors.Color(0.96, 0.96, 0.96)
                )
            )
        else:
        ...
        if track.greytrack_labels:
            ...
            for pos in range(self.start, self.end, labelstep):
                ...
                labelangle = (
                    self.sweep * 2 * pi *
                    (pos - self.start) / self.length - pi / 2
                )
                if theta > pi:
    ...
    def _draw_arc_poly(
        ...
        **kwargs,
        ...
        if (
            ...
            p.addArc(x0, y0, inner_radius, i_end,
                     i_start, moveTo=True, reverse=True)
            if flip:
                # Flipped, join end to start,
                self._draw_arc_line(
                    p, inner_radius, outer_radius, i_end, o_start)
                p.addArc(x0, y0, outer_radius, o_end, o_start, reverse=True)
                self._draw_arc_line(
                    p, outer_radius, inner_radius, o_end, i_start)
            else:
                # Not flipped, join start to start, end to end
                self._draw_arc_line(
                    p, inner_radius, outer_radius, i_end, o_end)
                p.addArc(x0, y0, outer_radius, o_end, o_start, reverse=False)
                self._draw_arc_line(
                    p, outer_radius, inner_radius, o_start, i_start)
            p.closePath()
            return p
        else:
            # Cheat and just use a four sided polygon.
            ...
            inner_endcos, inner_endsin = cos(
                inner_endangle), sin(inner_endangle)
            outer_startcos, outer_startsin = (
                cos(outer_startangle),
                sin(outer_startangle),
            )
            outer_endcos, outer_endsin = cos(
                outer_endangle), sin(outer_endangle)
            x1, y1 = (
    ...
    def _draw_sigil_cut_corner_box(
        ...
        **kwargs,
        ...
        startangle, endangle = min(
            startangle, endangle), max(startangle, endangle)
        ...
        cornerangle_delta = max(
            0.0, min(abs(boxheight) * 0.5 * corner /
                     middle_radius, abs(angle * 0.5))
        )
        if angle < 0:
        ...
        p.lineTo(x0 + shaft_inner_radius * startsin,
                 y0 + shaft_inner_radius * startcos)
        p.lineTo(x0 + shaft_outer_radius * startsin,
                 y0 + shaft_outer_radius * startcos)
        # Outer curved edge
        p.addArc(
            self.xcenter,
            self.ycenter,
        ...
        # Corner edges
        p.lineTo(x0 + shaft_outer_radius * endsin,
                 y0 + shaft_outer_radius * endcos)
        p.lineTo(x0 + shaft_inner_radius * endsin,
                 y0 + shaft_inner_radius * endcos)
        p.closePath()
        return p
    ...
    def _draw_arc_arrow(
        ...
        **kwargs,
        ...
        headangle_delta = max(
            0.0, min(abs(boxheight) * head_length_ratio /
                     middle_radius, abs(angle))
        )
        if angle < 0:
        ...
        if 0.5 >= abs(angle) and abs(headangle_delta) >= abs(angle):
            ...
            if abs(angle) < 0.5:
                p.lineTo(x0 + outer_radius * headsin,
                         y0 + outer_radius * headcos)
                p.lineTo(x0 + middle_radius * endsin,
                         y0 + middle_radius * endcos)
                p.lineTo(x0 + inner_radius * headsin,
                         y0 + inner_radius * headcos)
            else:
                self._draw_arc_line(
                    p,
                    outer_radius,
    ...
    def _draw_sigil_jaggy(
        ...
        **kwargs,
        ...
        if head_length_ratio and tail_length_ratio:
            headangle = max(
                endangle
                - min(height * head_length_ratio /
                      (center * teeth), angle * 0.5),
                startangle,
            )
            tailangle = min(
                startangle
                + min(height * tail_length_ratio /
                      (center * teeth), angle * 0.5),
                endangle,
            )
            ...
            headangle = max(
                endangle - min(height * head_length_ratio /
                               (center * teeth), angle),
                startangle,
            )
            ...
            headangle = endangle
            tailangle = min(
                startangle + min(height * tail_length_ratio /
                                 (center * teeth), angle),
                endangle,
            )
