...
class LinearDrawer(AbstractDrawer):
    ...
    def draw(self):
        ...
        for track_level in self.drawn_tracks:  # only use tracks to be drawn
            ...
            gbgs, glabels = self.draw_greytrack(
                track)  # get greytrack elements
            greytrack_bgs.append(gbgs)
            greytrack_labels.append(glabels)
            features, flabels = self.draw_track(
                track)  # get feature and graph elements
    ...
    def init_fragments(self):
        ...
        for marker in range(int(self.start), int(self.end), int(fragment_step)):
            self.fragment_limits[fragment_count] = (
                marker, marker + fragment_step)
            fragment_count += 1
    ...
    def set_track_heights(self):
        ...
        # The 'highest' track number to draw
        ...
        for track in trackunits:
            top = trackunits[track][1] * \
                trackunit_height - track_crop  # top offset
            btm = trackunits[track][0] * trackunit_height + \
                track_crop  # bottom offset
    ...
    def draw_feature_set(self, set):
        ...
        for feature in set.get_features():
            if self.is_in_bounds(feature.start) or self.is_in_bounds(feature.end):
                features, labels = self.draw_feature(
                    feature)  # get elements and labels
    ...
    def draw_feature(self, feature):
        ...
        for locstart, locend in feature.locations:
            ...
            feature_boxes = self.draw_feature_location(
    ...
    def draw_cross_link(self, cross_link):
        ...
        answer = []
        for fragment in range(
            min(start_fragmentA, start_fragmentB), max(
                end_fragmentA, end_fragmentB) + 1
        ):
            ...
            if fragment < start_fragmentB or end_fragmentB < fragment:
                ...
                (crop_leftA and not crop_rightA) or (
                ...
                (crop_rightA and not crop_leftA) or (
        ...
        for pos0, pos1, val in data:
            ...
            if fragment0 == fragment1:  # Box is contiguous on one fragment
                ...
                while self.fragment_limits[fragment][1] <= pos1:
                    ...
                    heat_elements.append(
                        draw_box(
                            (start_x, tbtm), (self.xlim,
                                              ttop), color=heat, border=None
                        )
                    )
                    fragment += 1
                    start_x = self.x0
        ...
        for pos0, pos1, val in data:
            ...
            if fragment0 == fragment1:  # Box is contiguous
                ...
                while self.fragment_limits[fragment][1] < pos1:
                    ...
                    bar_elements.append(
                        draw_box((start, tctr), (self.xlim,
                                 thisbarval), color=barcolor)
                    )
                    fragment += 1
                    start = self.x0
