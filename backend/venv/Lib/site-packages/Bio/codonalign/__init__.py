    ...
    if match:
        ...
        if match:
            ...
            if first_anchor and anchor_pos[0][2] != 0:
                shift_val_lst = [1, 2, 3 * anchor_len -
                                 2, 3 * anchor_len - 1, 0]
                sh_anc = anchors[0]
                for shift_val in shift_val_lst:
                    ...
                    if shift_val in (1, 2):
                        ...
                        sh_nuc_len = anchor_len * 3 - \
                            (3 * anchor_len - shift_val)
            ...
            for i in range(len(anchor_pos) - 1):
                shift_val = (anchor_pos[i + 1][0] -
                             anchor_pos[i][0]) % (3 * anchor_len)
                sh_anc = "".join(
                    anchors[anchor_pos[i][2]: anchor_pos[i + 1][2]])
    ...
    if mode in (0, 1):
        ...
        for aa in pro.seq:
            if aa == "-":
                ...
                if not re.search(
                    _codons2re(codon_table.start_codons), str(
                        this_codon.upper())
                ):
                    max_score -= 1
                    warnings.warn(
                ...
                this_codon = nucl_seq[span[0] + 3 *
                                      aa_num: span[0] + 3 * (aa_num + 1)]
        ...
        while True:
            ...
            if i in shift_start and m_groupdict[shift_start.index(i)].isupper():
                ...
                shift_val = 6 - (shift_pos[shift_index]
                                 [1] - shift_pos[shift_index][0])
                rf_table.append(i)
...
def _align_shift_recs(recs):
    ...
    if len(set(rf_num)) != 1:
        raise RuntimeError(
            "Number of alignable codons unequal in given records")
    i = 0
    rec_num = len(recs)
    while True:
        add_lst = []
