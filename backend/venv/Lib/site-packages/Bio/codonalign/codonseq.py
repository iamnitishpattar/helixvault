...
def _get_codon_fold(codon_table):
    ...
    for codon in codon_table.forward_table:
        if "U" not in codon:
            fold_table[codon] = find_fold_class(
                codon, codon_table.forward_table)
    fold_table["---"] = "---"
    return fold_table
    ...
    for n, (i, j) in enumerate(zip(codon1, codon2)):
        ...
        if i != j and (i in pyrimidine and j in pyrimidine):
            if fold_num[n] == "0":
                ...
                raise RuntimeError("Unexpected fold_num %d" % fold_num[n])
        if i != j and (
            (i in purine and j in pyrimidine) or (
    ...
    if codon1 == codon2:
        ...
        if len(diff_pos) == 1:
            ...
            for p in paths:
                ...
                tmp_codon.append((tmp1, tmp2))
                codon_idx = list(
                    map(codon_lst.index, [codon1, tmp1, tmp2, codon2]))
                prob = (
