            ...
            if x == clist[0] + i * step:  # are we still in the right step?
                ...
                if len(sub) == 1:
                    ...
                    if step == 1:
                        ...
                        shortlist.append("%d-%d\\%d" %
                                         (sub[0] + 1, sub[-1] + 1, step))
                clist = clist[i:]
                break
...
def combine(matrices):
    ...
    if not matrices:
        return None
    name = matrices[0][0]
    # initiate with copy of first matrix
        ...
        if self.command in SPECIAL_COMMANDS:
            ...
            if len(options) > 0:
                try:
                    ...
                    token_indices = [n for n in range(
                        len(options)) if n not in indices]
                    ...
                    raise NexusError(
                        f"Incorrect formatting in line: {line}") from None
...
class Nexus:
    ...
    def read(self, input):
        ...
        try:
            ...
            if isinstance(input, str):
                ...
                raise NexusError(
                    f"Unrecognized input: {input[:100]} ...") from None
    ...
    def _charstatelabels(self, options):
        ...
        while True:
            ...
            if c is None:
                ...
                if state is None:
                    raise NexusError(
                        f"Missing character state in line {options}.")
            ...
            if self.datatype != "standard":
                iupac_seq = Seq(
                    _replace_parenthesized_ambigs(
                        chars, self.rev_ambiguous_values)
                )
    ...
    def _tree(self, options):
        ...
        if opts.next_nonwhitespace() != "=":
            raise NexusError(
                f"Syntax error in tree description: {options[:50]}")
        rooted = False
        weight = 1.0
        while opts.peek_nonwhitespace() == "[":
            ...
            if special == "R":
                ...
                weight = float(value)
        tree = Tree(name=name, weight=weight,
                    rooted=rooted, tree=opts.rest().strip())
    ...
    def _codonposset(self, options):
        ...
        codonname = [
            n for n in self.charpartitions if n not in prev_partitions]
    ...
    def _name_n_vector(self, opts, separator="="):
        ...
        if opts.peek_nonwhitespace == "(":
            ...
            if qualifier.lower() == "vector":
                ...
                raise NexusError(
                    f"Unknown qualifier {qualifier} in line {opts}")
    ...
    def _resolve(self, identifier, set_type=None):
        ...
        if not set_type:
            raise NexusError(
                "INTERNAL ERROR: Need type to resolve identifier.")
        if set_type == CHARSET:
            try:
                n = int(identifier)
            except ValueError:
    ...
    def write_nexus_data(
        ...
        if interleave_by_partition:
            if interleave_by_partition not in self.charpartitions:
                raise NexusError(
                    f"Unknown partition: {interleave_by_partition!r}")
            else:
                ...
                for p in partition:
                    newpartition[p] = [
                        c for c in partition[p] if c not in exclude]
        ...
        with File.as_handle(filename, mode="w") as fh:
            ...
            namelength = max(len(safename(t, mrbayes=mrbayes))
                             for t in undelete)
            if interleave_by_partition:
                ...
                for p in names:
                    ...
                    if len(newpartition[p]) > 0:
                        for taxon in undelete:
                            fh.write(
                                safename(taxon, mrbayes=mrbayes).ljust(
                                    namelength + 1)
                            )
                            fh.write(
                ...
                for seek in range(0, nchar_adjusted, blocksize):
                    for taxon in undelete:
                        fh.write(safename(taxon, mrbayes=mrbayes).ljust(
                            namelength + 1))
                        fh.write(cropped_matrix[taxon]
                                 [seek: seek + blocksize] + "\n")
                    fh.write("\n")
            else:
                for taxon in undelete:
    ...
    def append_sets(
        ...
        if not codons_only:
            for n, ns in self.charsets.items():
                ...
                if cset:
                    setsb.append(
                        f"charset {safename(n)} = {_compact4nexus(cset)}")
            for n, s in self.taxsets.items():
                tset = [safename(t, mrbayes=mrbayes)
                        for t in s if t not in delete]
                if tset:
    ...
    def constant(self, matrix=None, delete=(), exclude=()):
        ...
        if not matrix:
            matrix = self.matrix
        undelete = [
            t for t in self.taxlabels if t in matrix and t not in delete]
        if not undelete:
            return None
        elif len(undelete) == 1:
    ...
    def cstatus(self, site, delete=(), narrow=True):
        ...
        for t in undelete:
            ...
            if narrow and c == self.missing:
                ...
                cstatus.extend(
                    b for b in self.ambiguous_values[c] if b not in cstatus)
    ...
    def insert_gap(self, pos, n=1, leftgreedy=False):
        ...
        for p in self.charpartitions:
            for sp, s in self.charpartitions[p].items():
                self.charpartitions[p][sp] = _adjust(
                    s, pos, n, leftgreedy=leftgreedy)
