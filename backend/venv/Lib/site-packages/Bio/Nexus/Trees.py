...
class Tree(Nodes.Chain):
    ...
    def _parse(self, tree):
        ...
        if tree.count("(") == 0:  # a leaf
            ...
            if colon == -1 and nodecomment == -1:  # none
                ...
                branch, comment = tree[:colon], self._get_values(
                    tree[colon + 1:])
            elif (
                colon < nodecomment
    ...
    def _get_values(self, text):
        ...
        if text == "":
            return None
        nodecomment = None
        # if there's a [&....] comment, cut it out
        if NODECOMMENT_START in text:
    ...
    def is_compatible(self, tree2, threshold, strict=True):
        ...
        if strict and (missing1 or missing2):
            ...
            raise TreeError(
                "Can't compare trees with different taxon compositions.")
        t1 = [
    ...
    def randomize(
        ...
        while len(terminals) < ntax:
            ...
            new_terminals = self.split(
                parent_id=newsplit, branchlength=branchlength)
    ...
    def display(self):
        ...
        table = [
            ("#", "taxon", "prev", "succ", "brlen",
             "blen (sum)", "support", "comment")
        ]
        ...
        print(
            "\n".join("%3s %32s %15s %15s %8s %10s %8s %20s" %
    ...
    def to_string(
        ...
        def newickize(node, ladderize=None):
            ...
            if not self.node(node).succ:  # terminal
                ...
                succnodes = ladderize_nodes(
                    self.node(node).succ, ladderize=ladderize)
                subtrees = [newickize(sn, ladderize=ladderize)
                            for sn in succnodes]
    ...
    def unroot(self):
        ...
        def _get_branches(node):
            ...
            for b in self.node(node).succ:
                branches.append(
                    [node, b, self.node(b).data.branchlength,
                     self.node(b).data.support]
                )
    ...
    def root_with_outgroup(self, outgroup=None):
        ...
        if len(oldroot) > 1:
            raise TreeError(
                f"Isolated nodes in tree description: {','.join(oldroot)}")
        elif len(oldroot) == 1:
            self.kill(oldroot[0])
        return self.root
    ...
    def merge_with_support(
        ...
        if not (bstrees or constree):
            raise TreeError(
                "Specify either list of bootstrap trees or consensus tree.")
        ...
        if bstrees:  # calculate consensus
            constree = consensus(
                bstrees, threshold=threshold, outgroup=outgroup)
        else:
        ...
        for pnode in self._walk():
            ...
            if cnode > -1:
                self.node(pnode).data.support = constree.node(
                    cnode).data.support
