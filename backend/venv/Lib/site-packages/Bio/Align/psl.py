...
"""  # noqa: W191, E101
    ...
    def format_alignment(self, alignment):
        ...
        if tCount == qCount:
            ...
            raise ValueError("inconsistent step sizes %d, %d" %
                             (tCount, qCount))
        # fmt: on
...
class AlignmentIterator(interfaces.AlignmentIterator):
    ...
    def _read_header(self, stream):
        ...
        if line.startswith("psLayout "):
            ...
            if words[1] != "version":
                raise ValueError(
                    "Unexpected word '%s' in header line" % words[1])
    ...
    def _read_next_alignment(self, stream):
        ...
        for line in lines:
            ...
            if len(words) == 23:
                ...
                raise ValueError(
                    "line has %d columns; expected 21 or 23" % len(words))
            strand = words[8]
            qName = words[9]
            qSize = int(words[10])
            tName = words[13]
            ...
            qStarts = [int(start)
                       for start in words[19].rstrip(",").split(",")]
            tStarts = [int(start)
                       for start in words[20].rstrip(",").split(",")]
            ...
            target_record = SeqRecord(
                target_sequence, id=tName, description="")
