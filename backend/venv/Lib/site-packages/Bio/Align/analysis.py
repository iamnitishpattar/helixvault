...
def _ng86(codons1, codons2, k, codon_table):
    ...
    S_sites1, N_sites1 = _count_site_NG86(
        codons1, codon_table=codon_table, k=k)
    S_sites2, N_sites2 = _count_site_NG86(
        codons2, codon_table=codon_table, k=k)
        ...
        if len(diff_pos) == 1:
            ...
            for index1, index2, index3 in paths:
                ...
                SN = [
                    ...
                    for i, j in zip(
                        SN, compare_codon(
                            codon1, tmp1, codon_table, weight=0.5 / 3)
                    )
                ]
                SN = [
                    i + j
                    for i, j in zip(
                        SN, compare_codon(
                            tmp1, tmp2, codon_table, weight=0.5 / 3)
                    )
                ]
                SN = [
                    i + j
                    for i, j in zip(
                        SN, compare_codon(
                            tmp2, codon2, codon_table, weight=0.5 / 3)
                    )
                ]
    return SN
...
def _yn00(codons1, codons2, codon_table):
    ...
    for temp in range(20):
        ...
        for (nucleotide1, nucleotide2), count in codon_npath.items():
            tv = _count_diff_YN00(nucleotide1, nucleotide2,
                                  P, codons, codon_table)
            TV = [m + n * count for m, n in zip(TV, tv)]
        TV = (TV[0] / S_sites, TV[1] /
              S_sites), (TV[2] / N_sites, TV[3] / N_sites)
...
def _count_diff_YN00(codon1, codon2, P, codons, codon_table):
    ...
    if codon1 == codon2:
        ...
        if len(diff_pos) == 1:
            ...
            tmp_codons = [codon1[:i] + codon2[i] + codon1[i + 1:]
                          for i in diff_pos]
            path_prob = []
            ...
            for index1, index2, index3 in paths:
                ...
                codon_idx = list(
                    map(codons.index, [codon1, tmp1, tmp2, codon2]))
                prob = (
            ...
            for codon, j, k in zip(tmp_codons, path_prob, paths):
                TV = [
                    ...
                    for p, q in zip(
                        TV, count_TV(
                            codon1, codon[0], k[0], codon_table, weight=j / 3)
                    )
                ]
                TV = [
                    p + q
                ...
                TV = [
                    p + q
                    for p, q in zip(
                        TV, count_TV(codon[1], codon2, k[1],
                                     codon_table, weight=j / 3)
                    )
                ]
    return TV
...
def _get_pi(codons1, codons2, cmethod, codon_table):
    ...
    if cmethod == "F1x4":
        ...
        for codon in codon_table.forward_table.keys() + codon_table.stop_codons:
            if "U" not in codon:
                pi[codon] = fcodon[codon[0]] * \
                    fcodon[codon[1]] * fcodon[codon[2]]
    elif cmethod == "F3x4":
        ...
        for codon in list(codon_table.forward_table.keys()) + codon_table.stop_codons:
            if "U" not in codon:
                pi[codon] = (
                    fcodon[0][codon[0]] *
                    fcodon[1][codon[1]] * fcodon[2][codon[2]]
                )
    elif cmethod == "F61":
        for codon in codon_table.forward_table.keys() + codon_table.stop_codons:
            if "U" not in codon:
...
def _get_Q(pi, k, w, codons, codon_table):
    ...
    for i1, codon1 in enumerate(codons):
        for i2, codon2 in enumerate(codons):
            if i1 != i2:
                Q[i1, i2] = _q(codon1, codon2, pi, k, w,
                               codon_table=codon_table)
    ...
    for i in range(size):
        ...
        for j in range(i):
            ...
            pairwise_alignment = Alignment(
                pairwise_sequences, pairwise_coordinates)
    ...
    for codon1 in codons:
        ...
        for codon2 in codons:
            if codon1 == codon2:
                ...
                nonsyn_G[codon1][codon2] = _dijkstra(
                    graph_nonsyn, codon1, codon2)
...
def _count_replacement(codons, G):
    ...
    if len(codons) == 1:
        ...
        subgraph = {
            codon1: {codon2: G[codon1][codon2]
                     for codon2 in codons if codon1 != codon2}
            for codon1 in codons
        }
