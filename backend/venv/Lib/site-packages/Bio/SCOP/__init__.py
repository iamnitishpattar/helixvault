...
class Scop:
    ...
    def getAscendentFromSQL(self, node, type):
        ...
        cur = self.db_handle.cursor()
        cur.execute(
            "SELECT " + type + " from cla WHERE " +
            node.type + "=%s", (node.sunid)
        )
        result = cur.fetchone()
        ...
        if type != "px":
            ...
            for d in data:
                if int(d[0]) not in self._sunidDict:
                    ...
                    cur.execute(
                        "SELECT parent FROM hie WHERE child=%s", n.sunid)
                    n.parent = cur.fetchone()[0]

                    cur.execute(
                        "SELECT child FROM hie WHERE parent=%s", n.sunid)
                    children = []
                    for c in cur.fetchall():
                        children.append(c[0])
                    n.children = children
...
class Astral:
    ...
    def domainsClusteredByEv(self, id):
        ...
        if id not in self.EvDatasets:
            if self.db_handle:
                self.EvDatasets[id] = self.getAstralDomainsFromSQL(
                    astralEv_to_sql[id])
    ...
    def domainsClusteredById(self, id):
        ...
        if id not in self.IdDatasets:
            if self.db_handle:
                self.IdDatasets[id] = self.getAstralDomainsFromSQL(
                    "id" + str(id))
            else:
                if not self.path:
                    raise RuntimeError("No scopseq directory specified")
    ...
    def writeToSQL(self, db_handle):
        ...
        cur.execute(
            "CREATE TABLE astral (sid CHAR(8), seq TEXT, PRIMARY KEY (sid))")
        ...
        for i in astralBibIds:
            ...
            for d in self.domainsClusteredById(i):
                cur.execute("UPDATE astral SET id" +
                            str(i) + "=1  WHERE sid=%s", d.sid)

        for ev in astralEvs:
            cur.execute("ALTER TABLE astral ADD (" +
                        astralEv_to_sql[ev] + " TINYINT)")

            for d in self.domainsClusteredByEv(ev):
                cur.execute(
                    "UPDATE astral SET " +
                    astralEv_to_sql[ev] + "=1  WHERE sid=%s",
                    d.sid,
                )
