                    ...
                    for line in stream:
                        ...
                        if line.startswith("COMMENT{"):
                            if line[8:].strip():
                                ...
                                record.reads[-1].rt[-1].comment.append(
                                    line[8:])
                            for line in stream:
                                line = line.strip()
                                if line.endswith("C}"):
...
def parse(source):
    ...
    try:
        ...
        if handle.read(0) != "":
            raise ValueError(
                "Ace files must be opened in text mode.") from None
    ...
    def sort(self):
        ...
        for i, c in enumerate(self.contigs):
            ...
            for j, r in enumerate(c.reads):
                if r.rt:
                    newrts = [
                        rt_tag for rt_tag in r.rt if rt_tag.name != r.rd.name]
                    for x in newrts:
                        self.contigs[i].reads[j].rt.remove(x)
                    rt.extend(newrts)
                if r.wr:
                    newwrs = [
                        wr_tag for wr_tag in r.wr if wr_tag.name != r.rd.name]
                    for x in newwrs:
