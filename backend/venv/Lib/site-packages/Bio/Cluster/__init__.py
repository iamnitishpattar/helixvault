...
def clustercentroids(data, mask=None, clusterid=None, method="a", transpose=False):
    ...
    _cluster.clustercentroids(data, mask, clusterid,
                              method, transpose, cdata, cmask)
    return cdata, cmask
...
class Record:
    ...
    def save(self, jobname, geneclusters=None, expclusters=None):
        ...
        if isinstance(geneclusters, Tree):
            ...
            geneindex = self._savekmeans(
                kggfilename, geneclusters, gorder, False)
            postfix = "_G%d" % k
        else:
    ...
    def _savedata(self, jobname, gid, aid, geneindex, expindex):
        ...
        with open(jobname + ".cdt", "w") as outputfile:
            ...
            for i in geneindex:
                ...
                outputfile.write(
                    f"{self.geneid[i]}\t{genename[i]}\t{gweight[i]:f}")
                for j in expindex:
                    outputfile.write("\t")
                    if mask[i, j]:
...
def __check_distancematrix(distancematrix):
    ...
    if isinstance(distancematrix, np.ndarray):
        distancematrix = np.require(
            distancematrix, dtype="d", requirements="C")
    else:
        try:
            ...
            for i, row in enumerate(distancematrix):
                ...
                if row.ndim != 1:
                    raise ValueError(
                        "row %d is not one-dimensional" % i) from None
                m = len(row)
                if m != i:
                    raise ValueError(
                        "row %d has incorrect size (%d, expected %d)" % (
                            i, m, i)
                    ) from None
                if np.isnan(row).any():
                    raise ValueError(
                        "distancematrix contains NaN values") from None
                d[i] = row
            return d
