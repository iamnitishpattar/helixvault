    ...
    def match(node):
        if "terminal" in kwargs:
            ...
            if pattern is not None and (
                not hasattr(
                    node, "is_terminal") or node.is_terminal() != pattern
            ):
                return False
        else:
            kwa_copy = kwargs
...
def _object_matcher(obj):
    ...
    raise ValueError(
        f"{obj} (type {type(obj)}) is not a valid type for comparison.")
...
class TreeMixin:
    ...
    def _filter_search(self, filter_func, order, follow_attrs):
        ...
        if follow_attrs:
            ...
            def get_children(elem): return elem.clades  # noqa: E731
            root = self.root
    ...
    def depths(self, unit_branch_lengths=False):  # noqa: D402
        ...
        if unit_branch_lengths:
            def depth_of(c): return 1  # noqa: E731
        else:
            def depth_of(c): return c.branch_length or 0  # noqa: E731
        depths = {}
    ...
    def ladderize(self, reverse=False):
        ...
        """
        self.root.clades.sort(
            key=lambda c: c.count_terminals(), reverse=reverse)
...
class BranchColor:
    ...
    def from_hex(cls, hexstr):
        ...
        """
        assert (
            isinstance(hexstr, str) and hexstr.startswith(
                "#") and len(hexstr) == 7
