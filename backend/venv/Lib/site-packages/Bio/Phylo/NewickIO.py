...
class Parser:
    ...
    def __init__(self, handle):
        ...
        if handle.read(0) != "":
            raise ValueError(
                "Newick files must be opened in text mode") from None
        self.handle = handle
        ...
        for match in tokens:
            ...
            if token.startswith("'"):
                ...
                if self.comments_are_confidence:
                    ...
                    current_clade.confidence = _parse_confidence(
...
class Writer:
    ...
    def to_strings(
        ...
        def newickize(clade):
            ...
            label = clade.name or ""
            if label:
                unquoted_label = re.match(
                    token_dict["unquoted node label"], label)
