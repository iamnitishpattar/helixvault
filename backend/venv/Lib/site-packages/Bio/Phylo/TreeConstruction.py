...
class DistanceMatrix(_Matrix):
    ...
    def format_phylip(self, handle):
        ...
        value_fmts = (
            "{" + str(x) + ":.4f}" for x in range(1, len(self.matrix) + 1))
...
class DistanceCalculator:
    ...
    def __init__(self, model="identity", skip_letters=None):
        ...
        if model == "identity":
            ...
            raise ValueError(
                "Model not supported. Available models: " +
                ", ".join(self.models)
            )
...
class DistanceTreeConstructor(TreeConstructor):
    ...
    def _height_of(self, clade):
        ...
        if clade.is_terminal():
            ...
            height = max(self._height_of(c) +
                         c.branch_length for c in clade.clades)
                ...
                for clade in tree.get_nonterminals(order="postorder"):
                    ...
                    for m in range(length):
                        ...
                        for n in range(length):
                            sl = self.matrix[alphabet[m],
                                             alphabet[n]] + left_score[n]
                            sr = self.matrix[alphabet[m],
                                             alphabet[n]] + right_score[n]
                            if min_l > sl:
                                min_l = sl
                            if min_r > sr:
                                min_r = sr
    ...
    def build_tree(self, alignment):
        ...
        if self.starting_tree is None:
            dtc = DistanceTreeConstructor(
                DistanceCalculator("identity"), "upgma")
