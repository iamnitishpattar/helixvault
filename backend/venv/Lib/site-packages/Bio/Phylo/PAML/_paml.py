...
class Paml:
        ...
        if alignment is not None:
            if not os.path.exists(alignment):
                raise FileNotFoundError(
                    "The specified alignment file does not exist.")
    ...
    def _set_rel_paths(self):
        ...
        if self.alignment is not None:
            self._rel_alignment = os.path.relpath(
                self.alignment, self.working_dir)
        if self.out_file is not None:
            self._rel_out_file = os.path.relpath(
                self.out_file, self.working_dir)
    ...
    def run(self, ctl_file, verbose, command):
        ...
        if ctl_file is None:
            ...
            if not os.path.exists(ctl_file):
                raise FileNotFoundError(
                    "The specified control file does not exist.")
        if verbose:
            result_code = subprocess.call([command, ctl_file])
        else:
            with open(os.devnull) as dn:
                result_code = subprocess.call(
                    [command, ctl_file], stdout=dn, stderr=dn)
        os.chdir(cwd)
        if result_code > 0:
        ...
        if result_code < 0:
            ...
            raise OSError(
                "The %s process was killed (return code %i)." % (
                    command, result_code)
            )
