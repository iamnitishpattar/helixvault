...
class Record:
    ...
    def _name(self):
        return _write_kegg(
            "NAME", [_wrap_kegg(line, wrap_rule=name_wrap)
    ...
    def _sysname(self):
        return _write_kegg(
            "SYSNAME", [_wrap_kegg(line, wrap_rule=name_wrap)
                        for line in self.sysname]
        )

    def _reaction(self):
        return _write_kegg(
            "REACTION", [_wrap_kegg(line, wrap_rule=rxn_wrap)
                         for line in self.reaction]
        )
    ...
    def _product(self):
        return _write_kegg(
            "PRODUCT", [_wrap_kegg(line, wrap_rule=name_wrap)
    ...
    def _comment(self):
        return _write_kegg(
            "COMMENT", [_wrap_kegg(line, wrap_rule=id_wrap(0))
    ...
    def _structures(self):
        ...
        for entry in self.structures:
            s.append(entry[0] + ": " + "  ".join(entry[1]) + "  ")
        return _write_kegg(
            "STRUCTURES", [_wrap_kegg(
                line, wrap_rule=struct_wrap(5)) for line in s]
        )
