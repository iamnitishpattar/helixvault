    ...
    def _load_index(self):
        ...
        try:
            ...
            if self._length != int(count):
                con.close()
                raise ValueError(
                    "Corrupt database? %i entries not %i" % (
            ...
            if filenames_relative_to_index:
                ...
                for f in self._filenames:
                    if os.path.isabs(f):
                        ...
                        tmp.append(
                            os.path.join(
                                relative_path, f.replace("/", os.path.sep))
                        )
                self._filenames = tmp
                del tmp
            ...
            con.close()
            raise ValueError(
                f"Not a Biopython index database? {err}") from None
    ...
    def _build_index(self):
        ...
        for file_index, filename in enumerate(filenames):
            ...
            if not os.path.isabs(filename) and not os.path.isabs(index_filename):
                ...
                f = os.path.relpath(filename, relative_path).replace(
                    os.path.sep, "/")
    ...
    def __getitem__(self, key):
        ...
        if file_number in proxies:
            ...
            proxy = self._proxy_factory(
                self._format, self._filenames[file_number])
