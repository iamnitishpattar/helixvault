...
class RestrictionType(type):
    ...
    def __mod__(cls, other):
        ...
        if not isinstance(other, RestrictionType):
            raise TypeError(
                f"expected RestrictionType, got {type(other)} instead")
...
class AbstractCut(RestrictionType):
    ...
    def _drop(cls):
        ...
        if cls.dna.is_linear():
            ...
            cls.results = [
                cut for cut in cls.results if filtering_function(cut)]
        else:
    ...
    def elucidate(cls):
        ...
        if cls.cut_twice():
            ...
            if f3 == 0:
                ...
                re = "N_" + abs(f3 - f5 + length) * "N" + \
                    "^" + abs(f5) * "N" + site
            else:
                re = "N_" + abs(f3 + length) * "N" + site + \
                    (f5 - length) * "N" + "^N"
        return re
    ...
    def search(self, dna, linear=True):
        ...
        if isinstance(dna, DNA):
            ...
            if (str(dna), dna.linear) == self.already_mapped:
                ...
                return self.mapping
        raise TypeError(
            f"Expected Seq or MutableSeq instance, got {type(dna)} instead")
    ...
    def with_name(self, names, dct=None):
        ...
        for i, enzyme in enumerate(names):
            if enzyme not in AllEnzymes:
                warnings.warn(
                    f"no data for the enzyme: {enzyme}", BiopythonWarning)
                del names[i]
        if not dct:
