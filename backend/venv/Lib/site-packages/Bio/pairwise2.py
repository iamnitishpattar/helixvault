...
class align:
    ...
    class alignment_function:
        ...
        def __init__(self, name):
            ...
            align_type, match_type, penalty_type = name[:-
                                                        2], name[-2], name[-1]
            try:
...
"""
        ...
        def decode(self, *args, **keywds):
            ...
            while i < len(self.param_names):
                if self.param_names[i] in [
                    ...
                    raise ValueError(
                        f"unknown parameter {self.param_names[i]!r}")
    ...
    def __getattr__(self, attr):
        ...
        new_alignment_function = type(
            "alignment_function", (object,), wrapper_dict)
...
def _align(
    ...
    if not alignments:
        ...
        score_matrix, trace_matrix = _reverse_matrices(
...
def _make_score_matrix_fast(
    ...
    """
    first_A_gap = calc_affine_penalty(
        1, open_A, extend_A, penalize_extend_when_opening)
    first_B_gap = calc_affine_penalty(
        1, open_B, extend_B, penalize_extend_when_opening)
    local_max_score = 0
    ...
    for i in range(1, lenB + 1):
        col_score.append(
            calc_affine_penalty(i, 2 * open_B, extend_B,
                                penalize_extend_when_opening)
        )
    ...
    while in_process and len(tracebacks) < MAX_ALIGNMENTS:
        ...
        if not dead_end:
            if not reverse:
                ...
                tracebacks.append(
                    (ali_seqB[::-1], ali_seqA[::-1], score, begin, end))
