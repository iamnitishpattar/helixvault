...
_SUPPORTED_FIELDS = set(
    list(_COLUMN_QRESULT) + list(_COLUMN_HIT) +
        ...
        while True:
            ...
            if prev is not None:
                ...
                for attr, value in prev["frag"].items():
                    ...
                    for seq_type in ("query", "hit"):
                        if attr == seq_type + "_start":
                            value = min(
                                value, prev["frag"][seq_type + "_end"]) - 1
                        elif attr == seq_type + "_end":
                            value = max(
                                value, prev["frag"][seq_type + "_start"])
                ...
                for seq_type in ("hit", "query"):
                    ...
                    strand = self._get_frag_strand(
                        frag, seq_type, prev["frag"])
...
class BlastTabIndexer(SearchIndexer):
    ...
    def __init__(self, filename, comments=False, fields=_DEFAULT_FIELDS):
        """Initialize the class."""
        SearchIndexer.__init__(
            self, filename, comments=comments, fields=fields)
