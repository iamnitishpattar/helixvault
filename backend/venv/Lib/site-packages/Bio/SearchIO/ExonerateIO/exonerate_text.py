...
def _get_row_dict(row_len, model):
    ...
    # 5 lines, translated dna vs translated dna
        ...
        raise ValueError(
            "Unexpected row count in alignment block: %i" % row_len)
    return idx
    ...
    for flag, parsed_len in zip(has_intron_after, raw_inter_lens):
        if flag:
            ...
            if all(parsed_len[:2]):
                ...
                intron_len = (
                    int(parsed_len[0]) if opp_type == "query" else int(
                        parsed_len[1])
                )
            # single hit/query introns
                ...
                raise ValueError(
                    "Unexpected intron parsing result: %r" % parsed_len)
        else:
            intron_len = 0
...
class ExonerateTextParser(_BaseExonerateParser):
    ...
    def parse_alignment_block(self, header):
        ...
        for annot_type in ("similarity", "query_annotation", "hit_annotation"):
            try:
                hsp["aln_annotation"][annot_type] = [x[annot_type]
                                                     for x in seq_blocks]
            except KeyError:
                pass
        ...
        if not has_ner:
            ...
            raw_inter_lens = re.findall(
                _RE_EXON_LEN, cmbn_rows[row_dict["midline"]])
        ...
        for seq_type in ("query", "hit"):
            ...
            if not has_ner:
                opp_type = "hit" if seq_type == "query" else "query"
                inter_lens = _comp_intron_lens(
                    seq_type, inter_blocks, raw_inter_lens)
            else:
            ...
            hsp["%s_ranges" % opp_type] = _comp_coords(
                hsp, opp_type, inter_lens)
