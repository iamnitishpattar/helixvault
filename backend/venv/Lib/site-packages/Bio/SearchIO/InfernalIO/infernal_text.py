    ...
    def _parse_qresult(self):
        ...
        while self.line:
            ...
            if self.line.startswith(_DIV_QUERY_START):
                ...
                qlen = int(re.sub(_RE_NOT_NUMERIC, "",
                           self.line.strip().split()[-1]))
        ...
        while True:
            ...
            # entering hit alignment block
                ...
                if self._meta["show alignments in output"] == "no":
                    ...
                    self._read_until(
                        lambda line: line.startswith(_DIV_TABLE_START))
                    self.line = read_forward(self.handle)
        ...
        while True:
            ...
            if (
                ...
                for i in range(len(local_aln_idx)):
                    ...
                    local_end = (
                        local_aln_idx[i + 1][0] if i +
                        1 < len(local_aln_idx) else None
                    )
                    ...
                    cur_hit_end = op(
                        cur_hit_start, len(
                            re.sub(_RE_LETTERS, "", cur_hit_seq))
                    )
                    ...
                    # annotations
                    cur_annot = {k: v[local_start:local_end]
