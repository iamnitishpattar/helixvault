...
def _create_hsp(hid, qid, psl):
    ...
    qstarts = _reorient_starts(
        psl["qstarts"], psl["blocksizes"], psl["qsize"], qstrand)
    # hit block starts
    if len(psl["strand"]) == 2:
    ...
    hit_range_all = list(
        zip(
            hstarts,
            [x + y * blocksize_multiplier for x,
                y in zip(hstarts, psl["blocksizes"])],
        )
    )
    ...
    def _build_row(self, qresult):
        ...
        for hit in qresult:
            for hsp in hit.hsps:
                ...
                # check spans
                eff_query_spans = [blocksize_multiplier *
                                   s for s in hsp.query_span_all]
                if hsp.hit_span_all != eff_query_spans:
                    raise ValueError(
                        "HSP hit span and query span values do not match.")
                ...
                if self.pslx:
                    line.append(",".join(str(x.seq)
                                for x in hsp.query_all) + ",")
                    line.append(",".join(str(x.seq)
                                for x in hsp.hit_all) + ",")
