...
class InterproscanXmlParser:
    ...
    def _parse_hit(self, root_hit_elem, query_id, query_seq=None):
        ...
        for hit_elem in root_hit_elem:
            ...
            hit_type = re.sub(r"%s(\w+)-match" %
                              self.NS, r"\1", hit_elem.find(".").tag)
            # store the hit id
            ...
            hsps = list(
                self._parse_hsp(
                    hit_elem.find(
                        self.NS + "locations"), query_id, hit_id, query_seq
                )
            )
    ...
    def _parse_xrefs(self, root_entry_elem):
        ...
        if root_entry_elem is not None:
            xref_elems = []
            xref_elems = xref_elems + \
                root_entry_elem.findall(self.NS + "go-xref")
            xref_elems = xref_elems + \
                root_entry_elem.findall(self.NS + "pathway-xref")
