...
class AsyncLRUCacheWrapper(Generic[P, T]):
    ...
    async def __call__(self, *args: P.args, **kwargs: P.kwargs) -> T:
        ...
        if self._typed:
            ...
            if kwargs:
                key += (initial_missing,) + tuple(type(val)
...
class _LRUCacheWrapper(Generic[T]):
    ...
    def __call__(
        ...
        return functools.lru_cache(maxsize=self._maxsize, typed=self._typed)(f)
...
def lru_cache(
    ...
    *,
    ...
    AsyncLRUCacheWrapper[P,
                         T] | functools._lru_cache_wrapper[T] | _LRUCacheWrapper[Any]
):
    """
...
async def reduce(  # type: ignore[misc]
    ...
    if isinstance(iterable, AsyncIterable):
        ...
        raise TypeError(
            "reduce() argument 2 must be an iterable or async iterable")
