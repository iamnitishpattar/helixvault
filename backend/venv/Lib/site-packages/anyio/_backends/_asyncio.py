...
if sys.version_info >= (3, 11):
    ...
    def _cancel_all_tasks(loop: AbstractEventLoop) -> None:
        ...
        loop.run_until_complete(tasks.gather(
...
class TaskGroup(abc.TaskGroup):
    ...
    def _spawn(
        ...
        if not iscoroutine(coro):
            prefix = f"{func.__module__}." if hasattr(
                func, "__module__") else ""
            raise TypeError(
        ...
        if sys.version_info >= (3, 14) and self.cancel_scope._host_task is not None:
            asyncio.future_add_to_awaited_by(
                task, self.cancel_scope._host_task)
...
class StreamWriterWrapper(abc.ByteSendStream):
    ...
    async def send(self, item: bytes) -> None:
        ...
        # type: ignore[attr-defined]
        stream_paused = self._stream._protocol._paused
        try:
